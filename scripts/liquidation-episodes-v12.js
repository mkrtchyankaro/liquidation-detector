 ...
 * node scripts/liquidation-episodes-v12.js [outputDays=2] [symbol=BTC] [rr=2.2]
...
function pathAfterEnd(buckets, endIdx, s, window) {
  ...
  if (!(p0 > 0) || endIdx + window >= buckets.length)
    return { cont: NaN, rev: NaN, contMin: NaN, revMin: NaN, first: "n/a" };
  let cont = 0,
    rev = 0,
    contMin = 0,
    revMin = 0;
 ...
 *  LONG victims (price fell)  -> BUY at END, SL = lowest price during the episode
 *  SHORT victims (price rose) -> SELL at END, SL = highest price during the episode
...
function backtestFade(prices, e, rr) {
  const a = lowerBound(prices, e.start),
    z = lowerBound(prices, e.end);
  const i0 = z;
  if (a >= z || i0 >= prices.length) return { result: "NO_DATA" };
  const long = e.victim === "LONG";
  let sl = prices[a].price;
  for (let i = a; i < z; i++)
    sl = long ? Math.min(sl, prices[i].price) : Math.max(sl, prices[i].price);
  const entry = prices[i0].price,
    entryTs = prices[i0].ts;
...
async function main() {
  ...
  try {
    ...
    if (!firstLiq || !lastLiq || !firstOi || !lastOi) {
      const [liqSymbols, oiSymbols] = await Promise.all([
        liq.distinct("symbol"),
        oi.distinct("symbol"),
      ]);
      const both = liqSymbols.filter((x) => oiSymbols.includes(x)).sort();
      throw Error(
        `${SYMBOL} liquidation/OI history missing. Symbols with both: ${both.join(", ") || "none"}`,
      );
    }
    ...
    if (until <= from)
      throw Error(`No overlapping ${SYMBOL} liquidation and OI history`);
    ...
    const prices = rawOi
      .map((x) => ({ ts: time(x.timestamp), price: num(x.price) }))
      .filter((x) => Number.isFinite(x.ts) && x.price > 0)
      .sort((p, q) => p.ts - q.ts);
    ...
    const flags = (c) =>
      Object.entries(c)
        ...
        .join(" ");
    console.log(
      `${SYMBOL} V12 OFFLINE EPISODE FILTER (V6 EPISODES; DOM + DIR + CLR + MOV + EXH; NO FIXED THRESHOLDS) (READ ONLY)`,
    );
    console.log(
    ...
    console.log(
      "Checks: DOM=victim side wins every part | DIR=price moved with liquidation | CLR=OI drop per 1% move > median | MOV=move >= median | EXH=after peak liq, less price move per 1% OI drop than before",
    );
    console.log(
      "EFF = price move % per 1% OI drop, before peak → after peak. AFTER60 = move 60m after END in fade direction. CONT/REV = max continuation / max reversal within 120m after END (minutes in brackets). Outcome only.",
    );
    console.log(
      "START UTC         END UTC           MIN  SIDE   VICTIM LIQ     OPP LIQ    MOVE%    OI DROP   CLR   EFF pre→post   CHECKS               SEL  AFTER60  CONT(min)        REV(min)         FIRST",
    );
    const row = (e) =>
    ...
    for (const e of sel)
      console.log(
        `  ${stamp(e.start).slice(0, 16)} → ${stamp(e.end).slice(0, 16)}  ${e.victim.padEnd(5)}  liq ${readable(e.victimLiq)}  move ${percent(e.priceMovePct)}  CLR ${x(e.clr)}  EFF ${x(e.pre.eff)}→${x(e.post.eff)}  cont ${percent(e.cont)} (${e.contMin}m)  rev ${percent(e.rev)} (${e.revMin}m)  first ${e.first}${e.rightCensored ? " OPEN" : ""}`,
      );
    ...
    const groupStats = (name, list) => {
      ...
      console.log(
        `  ${name.padEnd(22)} n=${String(ok.length).padStart(3)}  avg REV ${percent(avg(ok.map((e) => e.rev)))}  avg CONT ${percent(avg(ok.map((e) => e.cont)))}  REV first ${ok.length ? Math.round((100 * revFirst) / ok.length) : "n/a"}%`,
      );
    };
    console.log(
    ...
    groupStats(
      "SELECTED ✅",
      recent.filter((e) => e.selected),
    );
    groupStats(
      "DIR but rejected",
      recent.filter((e) => e.checks.DIR && !e.selected),
    );
    groupStats("all episodes", recent);

    // Fade backtest at END: SL = episode extreme, TP = RR * risk.
    for (const e of recent) e.trade = backtestFade(prices, e, RR);
    const tr = (e) => e.trade;
    console.log(
      `\nFADE BACKTEST (entry at END, SL = episode extreme, TP = ${RR}R, no fees):`,
    );
    console.log(
      "  START UTC         SIDE  TRADE  ENTRY UTC          ENTRY        SL           TP           RISK%    RESULT  MIN   SEL",
    );
    for (const e of recent) {
      const t = tr(e);
      if (!Number.isFinite(t.entry)) continue;
      const dur = Number.isFinite(t.exitTs)
        ? Math.round((t.exitTs - t.entryTs) / MINUTE)
        : "-";
      console.log(
        `  ${stamp(e.start).slice(0, 16)}  ${e.victim.padEnd(5)} ${(e.victim === "LONG" ? "BUY" : "SELL").padEnd(5)}  ${Number.isFinite(t.entryTs) ? stamp(t.entryTs).slice(0, 16) : "-".padEnd(16)}  ${readable(t.entry).padStart(11)}  ${readable(t.sl).padStart(11)}  ${readable(t.tp).padStart(11)}  ${percent(t.riskPct).padStart(7)}  ${t.result.padEnd(7)} ${String(dur).padStart(4)}  ${e.selected ? "✅" : ""}`,
      );
    }
    const btStats = (name, list) => {
      const done = list.filter(
        (e) => tr(e).result === "TP" || tr(e).result === "SL",
      );
      const tp = done.filter((e) => tr(e).result === "TP").length,
        sl = done.length - tp;
      const open = list.filter((e) => tr(e).result === "OPEN").length;
      const sumR = done.reduce((t, e) => t + tr(e).r, 0);
      console.log(
        `  ${name.padEnd(22)} trades=${String(done.length).padStart(3)}  TP=${String(tp).padStart(3)}  SL=${String(sl).padStart(3)}  open=${open}  win=${done.length ? ((100 * tp) / done.length).toFixed(1) : "n/a"}%  sumR=${sumR.toFixed(1)}  avgR=${done.length ? (sumR / done.length).toFixed(2) : "n/a"}`,
      );
    };
    console.log(
      `\nBACKTEST SUMMARY (break-even win rate at ${RR}R = ${(100 / (1 + RR)).toFixed(1)}%):`,
    );
    btStats(
      "SELECTED ✅",
      recent.filter((e) => e.selected),
