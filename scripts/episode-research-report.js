...
async function main() {
  ...
  for (const doc of docs) {
    ...
    if (isBtcConcurrent) {
      excluded.push({
        episodeId: doc.episodeId,
        symbol: doc.symbol,
        windowStartIso: isoUtc(windowStartMs),
        windowEndIso: isoUtc(windowEndMs),
        btcConcurrentLiquidatedUsd: btcUsd,
      });
    } else {
  ...
  console.log(`BTC-CONCURRENT (excluded): ${excluded.length}`);
  for (const e of excluded) {
    console.log(
      `  ${e.symbol.padEnd(10)} ${e.episodeId}  window ${e.windowStartIso} -> ${e.windowEndIso}  BTC liquidated in window: ${fmtUsd(e.btcConcurrentLiquidatedUsd)}`,
    );
  }
  ...
  for (const r of isolated) {
    ...
    console.log(
      `  episodeStart:  ${r.episodeStart.tsIso}  price=${fmtNum(r.episodeStart.price)}  spot=${fmtNum(r.episodeStart.spotMid)}  futures=${fmtNum(r.episodeStart.futuresMid)}  basis=${fmtNum(r.episodeStart.basisBps)}bps  OI=${fmtNum(r.episodeStart.oiValue)} (at ${r.episodeStart.oiTsIso ?? "N/A"})`,
    );
    console.log(
      `  finalExtreme:  ${r.finalExtreme.tsIso}  price=${fmtNum(r.finalExtreme.price)}  spot=${fmtNum(r.finalExtreme.spotMid)}  futures=${fmtNum(r.finalExtreme.futuresMid)}  basis=${fmtNum(r.finalExtreme.basisBps)}bps  OI=${fmtNum(r.finalExtreme.oiValue)} (at ${r.finalExtreme.oiTsIso ?? "N/A"})`,
    );
    console.log(
      `  ${r.entryOrEnd.label}:      ${r.entryOrEnd.tsIso}  price=${fmtNum(r.entryOrEnd.price)}  spot=${fmtNum(r.entryOrEnd.spotMid)}  futures=${fmtNum(r.entryOrEnd.futuresMid)}  basis=${fmtNum(r.entryOrEnd.basisBps)}bps  OI=${fmtNum(r.entryOrEnd.oiValue)} (at ${r.entryOrEnd.oiTsIso ?? "N/A"})`,
    );
    if (r.flushFlow) {
      console.log(
  ...
  for (const e of excluded)
    console.log(
      `  ${e.symbol} ${e.episodeId} window ${e.windowStartIso} -> ${e.windowEndIso} BTC=${fmtUsd(e.btcConcurrentLiquidatedUsd)}`,
    );
  ...
  for (const r of isolated) {
    ...
    console.log(
      `  start ${r.episodeStart.tsIso} price=${r.episodeStart.price} OI=${r.episodeStart.oiValue}`,
    );
    console.log(
      `  extreme ${r.finalExtreme.tsIso} price=${r.finalExtreme.price} OI=${r.finalExtreme.oiValue}`,
    );
    console.log(
      `  ${r.entryOrEnd.label} ${r.entryOrEnd.tsIso} price=${r.entryOrEnd.price} OI=${r.entryOrEnd.oiValue}`,
    );
    if (r.flushFlow)
      console.log(
