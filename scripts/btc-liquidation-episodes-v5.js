 ...
 * node scripts/btc-liquidation-episodes-v5.js [outputDays=2] [baselineDays=2]
...
async function main() {
  ...
  try {
    ...
    console.log(
      `BTC V5 OFFLINE OI-CHANGE-POINT RESEARCH (END = OI GROWTH FINISHED) (READ ONLY)`,
    );
    console.log(
    ...
    console.log(
      "Note: start = liquidation in OI-falling regime. End = OI stops growing (OI peak). Opposite-side liq is ignored. Speeds are OI %/min.",
    );
    console.log(
      "TIME START UTC                END UTC                  MIN  SIDE   LONG LIQ     SHORT LIQ    OI DROP   OI GROW  GROW MIN  SPEED 1st→last      PRICE START  PRICE END   MOVE%    EXTREME%  JOINT TIER    BASE N  END REASON      CONT  OPP",
    );
    for (const e of recent) {
      console.log(
        `${stamp(e.start)}  ${stamp(e.end)}  ${String(Math.round((e.end - e.start) / MINUTE)).padStart(3)}  ${e.victim.padEnd(5)}  ${String(readable(e.long)).padStart(11)}  ${String(readable(e.short)).padStart(11)}  ${percent(e.oiDropPct).padStart(8)}  ${percent(e.oiGrowPct).padStart(8)}  ${String(e.growthMin).padStart(8)}  ${(percent(e.firstGrowSpeed) + "→" + percent(e.lastGrowSpeed)).padStart(17)}  ${readable(e.startPrice).padStart(11)}  ${readable(e.endPrice).padStart(10)}  ${percent(e.priceMovePct).padStart(8)}  ${percent(e.extremeMovePct).padStart(9)}  ${readable(e.joint).padStart(5)} ${e.tier.padEnd(8)} ${String(e.baselineN).padStart(4)}  ${e.endReason.padEnd(14)}  ${String(e.continuations).padStart(4)}  ${String(e.opposite).padStart(3)}${e.leftCensored || e.rightCensored ? " CENSORED" : ""}`,
      );
    }
    console.log(
