...
//
//   node scripts/cascade-episodes-full-report-v2.js 3 9
//
// v2 CHANGES (operator-requested):
...
const PERCENTILE_LOOKBACK_HOURS = 24; // operator-requested: trailing 24h of this symbol's own liquidation EVENT sizes, as P90/P95 context next to each coin
...
async function main() {
  ...
  for (let epIdx = 0; epIdx < runs.length; epIdx++) {
    ...
    for (const symbol of SYMBOLS) {
      ...
      // Operator-requested: this symbol's own trailing-24h liquidation
      // EVENT-SIZE P90/P95, as context -- were this cascade's events
      // unusually large for this coin lately, or business as usual?
      const lookbackFrom =
      ...
      console.log(
        `  ${symbol.padEnd(10)} price ${fmtNum(startPrice)} -> ${fmtNum(endPrice)}  (${fmtPct(priceChangePct)})   liq=${fmtUsd(liqUsd)} (${events.length} events)`,
      );
      console.log(
        `    OI: ${fmtNum(oiStart)} -> ${fmtNum(oiEnd)}  (${fmtPct(oiDeltaPct)})   ATR(14,15m): start=${fmtNum(atrAtStart)} end=${fmtNum(atrAtEnd)}`,
      );
      console.log(
        `    Prior ${PERCENTILE_LOOKBACK_HOURS}h event-size P90/P95: ${fmtUsd(p90)} / ${fmtUsd(p95)}  (n=${priorEventSizes.length})   largest event this cascade: ${fmtUsd(events.length > 0 ? Math.max(...events.map((e) => e.usd)) : null)}`,
      );
      console.log(
