  ...
  try {
    ...
    const flags = (c) =>
      Object.entries(c)
        ...
        .join(" ");
    console.log(
      `${SYMBOL} V13 OFFLINE EPISODE FILTER (V6 EPISODES; DOM + DIR + CLR + MOV + EXH; NO FIXED THRESHOLDS) (READ ONLY)`,
    );
    console.log(
    ...
    for (const e of recent) {
      e.tEnd = backtestFade(prices, e, RR, e.end);
      e.tConf = backtestFade(prices, e, RR, e.confirmTs);
    }
    console.log(
      `\nFADE BACKTEST (SL = extreme from start until entry, TP = ${RR}R, no fees). CONFIRM = when the end becomes known.`,
    );
    console.log(
      "  START UTC         SIDE  TRADE  END UTC            CONFIRM UTC     DELAY  RISK% END  END RES   RISK% CONF  CONF RES  CONF MIN  SEL",
    );
    for (const e of recent) {
      const a = e.tEnd,
        b = e.tConf;
      if (!Number.isFinite(a.entry) && !Number.isFinite(b.entry)) continue;
      const delay = Number.isFinite(e.confirmTs)
        ? Math.round((e.confirmTs - e.end) / MINUTE)
        : "-";
      const dur = Number.isFinite(b.exitTs)
        ? Math.round((b.exitTs - b.entryTs) / MINUTE)
        : "-";
      console.log(
    ...
    for (const [key, label] of [
      ["tEnd", "ENTRY AT END (look-ahead, old v12)"],
      ["tConf", "ENTRY AT CONFIRM (realistic timing)"],
    ]) {
      console.log(
    ...
    console.log(
      "Still offline: OI regimes and medians use all loaded history. CONFIRM removes only the end-timing look-ahead.",
    );
  } finally {
    await client.close();
  }
