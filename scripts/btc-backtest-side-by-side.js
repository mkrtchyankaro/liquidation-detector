...
function buildPureCandleEpisodes(events, btc1m, btc3m, btc5m, maxSearchMs) {
  ...
  while (i < events.length) {
    ...
    episodes.push({
      dominantSide: events[epEvents.length ? 0 : 0]?.victim ?? "LONG",
      startMs,
      endMs,
...
async function main() {
  ...
  const pureTotals = pureEpisodes.map((e) => e.totalUsd).sort((a, b) => a - b);
  const confirmedTotals = confirmedEpisodes
    .map((e) => e.totalUsd)
    .sort((a, b) => a - b);
  const pureP90 = percentile(pureTotals, percentileThreshold);
  const confirmedP90 = percentile(confirmedTotals, percentileThreshold);
  const pureBig = pureEpisodes.filter((e) => e.totalUsd > pureP90);
  const confirmedBig = confirmedEpisodes.filter(
    (e) => e.totalUsd > confirmedP90,
  );

  console.log(`\n${"=".repeat(120)}`);
  console.log(
    `METHOD A -- PURE-CANDLE: ${pureEpisodes.length} total episodes, ${pureBig.length} big (>P${percentileThreshold}=${fmtUsd(pureP90)})`,
  );
  console.log("=".repeat(120));
  ...
  console.log(`\n${"=".repeat(120)}`);
  console.log(
    `METHOD B -- EXTREME-BREAK-CONFIRMED: ${confirmedEpisodes.length} total episodes, ${confirmedBig.length} big (>P${percentileThreshold}=${fmtUsd(confirmedP90)})`,
  );
  console.log("=".repeat(120));
