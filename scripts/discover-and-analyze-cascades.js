...
//
//   node scripts/discover-and-analyze-cascades.js 3
//
...
async function main() {
  ...
  for (let runIdx = 0; runIdx < runs.length; runIdx++) {
    ...
    for (const o of onsets) {
      ...
      console.log(
        `  ${o.symbol.padEnd(10)} lag=${fmtDuration(lagSec).padEnd(8)} +1m=${fmtPct(returns[0]).padEnd(10)} +5m=${fmtPct(returns[1]).padEnd(10)} +15m=${fmtPct(returns[2])}`,
      );
      aggregate.push({
        lagSec,
        returns,
  ...
  if (confirmedEpisodeCount === 0) {
    ...
    console.log(
      "LAG CATEGORY                  N     +1m mean(+-sd)         +5m mean(+-sd)         +15m mean(+-sd)",
    );
    console.log("-".repeat(112));
