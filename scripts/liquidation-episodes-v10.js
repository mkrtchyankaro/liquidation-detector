 ...
 * node scripts/liquidation-episodes-v10.js [outputDays=2] [symbol=BTC]
  ...
  try {
    ...
    const flags = (c) =>
      Object.entries(c)
        ...
        .join(" ");
    console.log(
      `${SYMBOL} V10 OFFLINE EPISODE FILTER (V6 EPISODES; DOM + DIR + CLR + MOV + EXH; NO FIXED THRESHOLDS) (READ ONLY)`,
    );
    console.log(
    ...
    console.log(
      "EFF = price move % per 1% OI drop, before peak → after peak. AFTER = price move after END in FADE direction (outcome only, uses future data).",
    );
    console.log(
      "START UTC         END UTC           MIN  SIDE   VICTIM LIQ     OPP LIQ    MOVE%    OI DROP   CLR   EFF pre→post   CHECKS               SEL  AFTER30  AFTER60  AFTER120",
    );
    const row = (e) =>
      `${stamp(e.start).slice(0, 16)}  ${stamp(e.end).slice(0, 16)}  ${String(Math.round((e.end - e.start) / MINUTE)).padStart(4)}  ${e.victim.padEnd(5)}  ${readable(e.victimLiq).padStart(13)}  ${readable(e.oppLiq).padStart(12)}  ${percent(e.priceMovePct).padStart(8)}  ${percent(e.oiDropPct).padStart(7)}  ${x(e.clr).padStart(6)}  ${(x(e.pre.eff) + "→" + x(e.post.eff)).padStart(13)}  ${flags(e.checks)}  ${e.selected ? " ✅" : "   "}  ${percent(e.after30).padStart(7)}  ${percent(e.after60).padStart(7)}  ${percent(e.after120).padStart(8)}${e.rightCensored ? " OPEN" : ""}`;
    for (const e of recent) console.log(row(e));
    const sel = recent.filter((e) => e.selected);
    console.log(`\nSELECTED (${sel.length}):`);
    for (const e of sel)
      console.log(
        `  ${stamp(e.start).slice(0, 16)} → ${stamp(e.end).slice(0, 16)}  ${e.victim.padEnd(5)}  liq ${readable(e.victimLiq)}  move ${percent(e.priceMovePct)}  CLR ${x(e.clr)}  EFF ${x(e.pre.eff)}→${x(e.post.eff)}  after60 ${percent(e.after60)}  after120 ${percent(e.after120)}${e.rightCensored ? " OPEN" : ""}`,
      );
