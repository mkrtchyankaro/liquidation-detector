 ...
 * node scripts/btc-liquidation-episodes-v3.js [outputDays=2] [baselineDays=2]
...
/**
 * OI-negative regimes containing forced liquidations are clearing episodes.
 * Adjacent negative regimes merge; zero-liquidation buckets inside them remain.
 * An episode finishes at its minimum OI before a nonnegative-slope regime.
 */
function episodesFromRegimes(buckets, regimes, validFrom, validUntil) {
  const episodes = [];
  let active = null;
  function close(boundary, rightCensored = false) {
    if (!active) return;
    const start = active.start, stop = Math.max(start, Math.min(boundary, buckets.length - 1));
    const included = buckets.slice(start, stop + 1);
    const long = included.reduce((s, b) => s + b.long, 0);
    const short = included.reduce((s, b) => s + b.short, 0);
    const victim = long >= short ? "LONG" : "SHORT";
  ...
  for (const regime of regimes) {
    if (regime.slope < 0) {
      const firstLiq = buckets.findIndex((b, i) => i >= regime.a && i < regime.b && b.count > 0);
      if (firstLiq !== -1 && !active) active = { start: firstLiq };
    } else if (active) {
      // Local minimum before the structural OI reversal (not a fixed wait).
      let minimumIndex = active.start;
      for (let i = active.start; i < regime.a; i++) {
        if (buckets[i].oi < buckets[minimumIndex].oi) minimumIndex = i;
      }
      close(minimumIndex);
    }
  }
...
async function main() {
  ...
  try {
    ...
    if (!firstLiq || !lastLiq || !firstOi || !lastOi) throw Error("BTC liquidation/OI history missing");
    ...
    if (until <= from) throw Error("No overlapping BTC liquidation and OI history");
    ...
    console.log(`BTC V3 OFFLINE OI-CHANGE-POINT RESEARCH (READ ONLY)`);
    ...
    console.log(`Source: ${events.length} forced-order snapshots; ${observations.length} OI polls; output=${outputDays}d; trailing baseline=${baselineDays}d`);
    console.log("Note: OI change points use all available history (offline look-ahead). Percentile reference sets exclude future episodes.");
    console.log("Note: only OI-clearing regimes with liquidation events are episodes; flat/rising OI with liquidation is NOT silently called clearing.");
    console.log("TIME START UTC                END UTC                  MIN  SIDE   LONG LIQ     SHORT LIQ    OI DROP   LIQ%  OI%   JOINT TIER  BASE N COVERAGE");
    for (const e of recent) {
      console.log(`${stamp(e.start)}  ${stamp(e.end)}  ${String(Math.round((e.end - e.start) / MINUTE)).padStart(3)}  ${e.victim.padEnd(5)}  ${String(readable(e.long)).padStart(11)}  ${String(readable(e.short)).padStart(11)}  ${percent(e.oiDropPct).padStart(8)}  ${readable(e.liqRank).padStart(5)}  ${readable(e.oiRank).padStart(5)}  ${readable(e.joint).padStart(5)} ${e.tier.padEnd(8)} ${String(e.baselineN).padStart(4)}  ${percent(e.coverage * 100)}${e.leftCensored || e.rightCensored ? " CENSORED" : ""}`);
    }
    console.log("\nCausal same-side P70/P75/P80/P90/P95 limits for recent episodes:");
    for (const e of recent.filter((x) => x.tier === "P80" || x.tier === "P90" || x.tier === "P95")) {
      console.log(`${stamp(e.start)} ${e.victim} ${e.tier} baselineN=${e.baselineN} ` + PERCENTILES.map((p) => `P${p}[$${readable(e.thresholds[p].liq)}, OI ${percent(e.thresholds[p].oi)}]`).join(" "));
    }
    const counts = Object.fromEntries(["UNRANKED", "<P70", ...PERCENTILES.map((p) => `P${p}`)].map((tier) => [tier, recent.filter((e) => e.tier === tier).length]));
    console.log(`\nRecent episodes=${recent.length}; tiers=${JSON.stringify(counts)}`);
    console.log("Do not use these offline boundaries as live trade signals without causal replay and chart review.");
  } finally { await client.close(); }
}

if (require.main === module) main().catch((error) => { console.error(error); process.exitCode = 1; });
module.exports = { buildBuckets, changePoints, episodesFromRegimes, rankEpisodes };
