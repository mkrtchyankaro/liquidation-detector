 ...
 * node scripts/btc-liquidation-episodes-v7.js [outputDays=2]
...
/**
 * V7 quality features (no percentiles). Four checks, each pass/fail:
 ...
 *  BLD  peak victim-liquidation minute is in the second half of the episode (build-up)
...
function episodeFeatures(buckets, e) {
  ...
  const checks = { DOM: dom, DIR: dirMove > 0, BLD: peakPos >= 0.5, EXH: exh };
  return {
    ...e,
    victimLiq,
    oppLiq,
...
async function main() {
  ...
  try {
    ...
    const x = (v) =>
      v === Infinity ? "∞" : Number.isFinite(v) ? v.toFixed(1) : "n/a";
    const flags = (c) =>
      Object.entries(c)
        .map(([k, v]) => (v ? k : "·".repeat(k.length)))
        .join(" ");
    console.log(
      `BTC V7 OFFLINE EPISODE QUALITY (V6 EPISODES + 4 FEATURES, NO PERCENTILES) (READ ONLY)`,
    );
    console.log(
    ...
    console.log(
      "Checks: DOM=victim side wins every part | DIR=price moved with liquidation | BLD=peak liq in 2nd half | EXH=after peak, less price per OI drop",
    );
    console.log(
      "AFTER = price move after END in the FADE direction (positive = reversal). Outcome only, uses future data.",
    );
    console.log(
      "START UTC            END UTC              MIN  SIDE   VICTIM LIQ   OPP LIQ     DOM×   MOVE%    OI DROP  PEAK UTC  POS   EFF pre→post    SCORE CHECKS           AFTER30  AFTER60  AFTER120",
    );
    const t = (ms) => stamp(ms).slice(5, 16).replace("T", " ");
    const row = (e) =>
      `${stamp(e.start).slice(0, 16)}  ${stamp(e.end).slice(0, 16)}  ${String(Math.round((e.end - e.start) / MINUTE)).padStart(4)}  ${e.victim.padEnd(5)}  ${readable(e.victimLiq).padStart(12)}  ${readable(e.oppLiq).padStart(10)}  ${x(e.domRatio).padStart(6)}  ${percent(e.priceMovePct).padStart(8)}  ${percent(e.oiDropPct).padStart(7)}  ${t(e.peakTs).slice(6)}     ${e.peakPos.toFixed(2)}  ${(x(e.pre.eff) + "→" + x(e.post.eff)).padStart(12)}  ${String(e.score).padStart(5)}  ${flags(e.checks)}  ${percent(e.after30).padStart(7)}  ${percent(e.after60).padStart(7)}  ${percent(e.after120).padStart(8)}${e.rightCensored ? " OPEN" : ""}`;
    for (const e of recent) console.log(row(e));
    for (const need of [4, 3]) {
      const list = recent.filter((e) => e.score === need);
      console.log(`\nSCORE ${need}/4 (${list.length}):`);
      for (const e of list)
        console.log(
          `  ${stamp(e.start).slice(0, 16)} → ${stamp(e.end).slice(11, 16)}  ${e.victim.padEnd(5)}  liq ${readable(e.victimLiq)}  move ${percent(e.priceMovePct)}  missing: ${
            Object.entries(e.checks)
              .filter(([, v]) => !v)
              .map(([k]) => k)
              .join(",") || "-"
          }  after60 ${percent(e.after60)}`,
        );
    }
    const byScore = {};
    for (const e of recent) byScore[e.score] = (byScore[e.score] ?? 0) + 1;
    console.log(
      `\nRecent episodes=${recent.length}; by score=${JSON.stringify(byScore)}`,
    );
    console.log(
