...
async function main() {
  ...
  // Sep 20 2026 (Karo), operator-reported BUG FIX -- v1 mixed
  // segmentation and the recovery search in the SAME loop, which
  // could interact and over-fragment the count (8 found instead of
  // the operator's known, correct 4). Fixed: segmentation is now a
  // SEPARATE, single pass (same 3h search cap as the originally
  // validated pure-candle method) that fully consumes the event list
  // FIRST -- the recovery search afterward is read-only annotation
  // and never touches segmentation.
    ...
    while (i < events.length && events[i].timestamp <= windowEndMs) {
      ...
      const candidateEndMs =
        findNextEndCandidate(
          ...
          btc3m,
          btc5m,
        ) ?? startEvent.timestamp + MAX_CANDIDATE_SEARCH_MIN * 60 * 1000;
      const epEvents = [];
      while (i < events.length && events[i].timestamp <= candidateEndMs) {
        epEvents.push(events[i]);
        i++;
      }
      fixedEpisodes.push({ direction, epEvents });
    }
  }
  console.log(
    `Segmented into ${fixedEpisodes.length} episode(s) (single pass, 3h search cap).\n`,
  );

  for (
    let candidateNum = 1;
    candidateNum <= fixedEpisodes.length;
    candidateNum++
  ) {
    const { direction, epEvents } = fixedEpisodes[candidateNum - 1];
    const startEvent = epEvents[0];
    ...
    // Recovery confirmation -- READ-ONLY annotation, never affects segmentation above.
