...
//
//   node scripts/cascade-episodes-full-report-v11.js 5 6 2000000
//
// v11 CHANGES -- self-caught bug fix before trusting v10's numbers:
//   - v10's direction-detection (goingUp) required that NO candle in
//     the window dip below start*0.999, which fails for nearly every
//     real bearish cascade (lows obviously go below start) and can
//     misfire on ordinary noise even in a genuine bullish one --
//     silently mislabeling direction would corrupt the candle-
//     confirmed end for however many cascades it hit. Replaced with a
//     robust comparison: whichever side (max reach up vs max reach
//     down from the cascade's own start price) is larger across the
//     whole window determines direction.
//   - CASCADE END redefined: the pure liquidation-$ exhaustion cutoff
//     is now only a FALLBACK/starting point. The real end is
//     CANDLE-CONFIRMED on BTC's own price action across 1m, 3m, AND
//     5m timeframes AT ONCE -- end is the point where BTC stops
//     making a new extreme (in the cascade's own direction) on all
//     three timeframes simultaneously, for 3 consecutive candles of
//     each ("the candles holding each other", operator's words). This
//     is much closer to a live-deployable rule than a raw $ threshold.
//   - BIG CASCADES ONLY: a new 4th CLI arg (default $2M) is a floor
//     on TOTAL liquidated $ across all participating symbols; smaller
//     cascades are dropped entirely before analysis.
//   - Default days back down to 5 (3-5 day backtests, per operator's
//     explicit urgency to reach a live decision soon).
//   - The REVERSAL/CONTINUATION/FLAT classification used a FIXED
//     +-0.10% threshold on normalized +60m recovery. That number
//     meant a huge move for BTC (low volatility) and a trivial one
//     for a volatile altcoin -- not comparable across coins. Fixed:
//     the threshold is now +-0.5x THAT COIN'S OWN ATR (expressed as
//     % of its own price at cascade end) -- it scales to each coin's
//     own normal volatility, computed fresh from that coin's own
//     data every time, never a fixed constant applied to all 10.
...
const DYNAMIC_THRESHOLD_ATR_MULTIPLIER = 0.5; // Sep 20 2026 (Karo), operator-requested -- v9: replaces the old FIXED 0.10% threshold, which meant a huge move for BTC and a trivial one for a volatile altcoin. Classification threshold is now 0.5x THAT COIN'S OWN ATR (as % of its own price at cascade end) -- scales to each coin's own normal volatility instead of one number for all 10 symbols.
...
async function main() {
  const days = Number(process.argv[2] ?? "5"); // Sep 20 2026 (Karo), operator-requested v10 -- default down to 5 (3-5 day backtests, moving toward live)
  const minDistinctSymbols = Number(process.argv[3] ?? "6");
  const minTotalCascadeUsd = Number(process.argv[4] ?? "2000000"); // Sep 20 2026 (Karo), operator-requested v10 -- BIG cascades only; total liquidation $ across all participating symbols must clear this floor or the cascade is dropped entirely, before any analysis
  ...
  // Sep 20 2026 (Karo), operator-requested v10 -- CANDLE-BASED cascade
  // end, replacing the pure liquidation-$ exhaustion cutoff, which the
  // operator flagged as not being what we'd actually run live: a
  // brief pause in the liquidation FEED (not in price itself) could
  // call a cascade "over" while BTC is still trending. New definition
  // uses BTC's OWN price action across THREE timeframes at once (1m,
  // 3m, 5m) -- end is confirmed only once BTC stops making a new
  // extreme (in the cascade's own direction) on ALL THREE timeframes
  // simultaneously for CANDLE_CONFIRM_COUNT consecutive candles of
  // each -- "the candles holding each other", in the operator's words.
  console.log(
    "Fetching BTC 3m/5m klines for candle-based end confirmation...\n",
  );
  const btc1m = priceCandlesBySymbol.get("BTCUSDT");
  ...
  const CANDLE_CONFIRM_COUNT = 3;
  const MAX_CANDLE_SEARCH_MS = 3 * 60 * 60 * 1000; // cap the forward search at 3h

  /** Returns the ms timestamp where BTC's price, across 1m/3m/5m
   *  candles all at once, stops making a new extreme in `direction`
   *  ("up" or "down") for CANDLE_CONFIRM_COUNT consecutive candles of
   *  EACH timeframe. Falls back to the liquidity-exhaustion end
   *  (`fallbackEndMs`) if BTC candles are unavailable or the search
   *  cap is reached without confirmation. */
  function findCandleConfirmedEndMs(extremeMs, direction, fallbackEndMs) {
    function noNewExtremeStreak(candles, fromMs) {
      const relevant = candles.filter(
        (c) =>
          c.openTimeMs >= extremeMs &&
          c.openTimeMs <= fromMs + MAX_CANDLE_SEARCH_MS,
      );
      if (relevant.length === 0) return null;
      let extremeVal = direction === "up" ? -Infinity : Infinity;
      for (const c of relevant) {
        if (c.openTimeMs > fromMs) break;
        extremeVal =
          direction === "up"
            ? Math.max(extremeVal, c.high)
            : Math.min(extremeVal, c.low);
      }
      let streak = 0;
      for (const c of relevant) {
        if (c.openTimeMs <= fromMs) continue;
        const madeNewExtreme =
          direction === "up" ? c.high > extremeVal : c.low < extremeVal;
        if (madeNewExtreme) {
          extremeVal =
            direction === "up"
              ? Math.max(extremeVal, c.high)
        ...
        if (streak >= CANDLE_CONFIRM_COUNT) return c.openTimeMs;
      }
      return null;
    }

    if (btc1m.length === 0 || btc3m.length === 0 || btc5m.length === 0)
      return fallbackEndMs;
    const end1m = noNewExtremeStreak(btc1m, extremeMs);
    const end3m = noNewExtremeStreak(btc3m, extremeMs);
    const end5m = noNewExtremeStreak(btc5m, extremeMs);
    if (end1m === null || end3m === null || end5m === null)
      return fallbackEndMs;
    return Math.max(end1m, end3m, end5m); // all three timeframes must agree -- take the latest (last to confirm)
  }

  let displayedCascadeCount = 0;
  for (let epIdx = 0; epIdx < mergedRuns.length; epIdx++) {
    ...
    const exhaustionEndMs = Math.min(
      rangeEndMs,
      rangeStartMs +
    ...
    // BIG CASCADES ONLY -- total liquidation $ across all symbols in
    // the exhaustion window must clear the operator-set floor, or
    // this cascade is skipped entirely before any further work.
    let totalCascadeUsd = 0;
    for (const s of SYMBOLS)
      totalCascadeUsd += eventsBySymbol
        .get(s)
        .filter((e) => e.ts >= cascadeStartMs && e.ts <= exhaustionEndMs)
        .reduce((a, e) => a + e.usd, 0);
    if (totalCascadeUsd < minTotalCascadeUsd) continue;

    // Refine the end using BTC's own candle-confirmed exhaustion: find
    // BTC's own extreme within [start, exhaustionEnd], determine
    // direction from that extreme vs the cascade start price, then
    // confirm the end only once BTC stops making a new extreme on
    // 1m/3m/5m all at once.
    let cascadeEndMs = exhaustionEndMs;
    {
      const startP = nearestPriceAtOrBefore(btc1m, cascadeStartMs);
      const windowCandles = btc1m.filter(
        (c) =>
          c.openTimeMs >= cascadeStartMs && c.openTimeMs <= exhaustionEndMs,
      );
      if (startP !== null && windowCandles.length > 0) {
        // Sep 20 2026 (Karo) -- CRITICAL FIX: the old direction check
        // required NO candle to dip below start*0.999, which fails
        // for almost any real bearish cascade (of course lows dip
        // below start) and can misfire on ordinary noise even in a
        // genuine bullish one. Replaced with a robust comparison:
        // whichever side (max upward reach vs max downward reach from
        // start) is bigger across the whole window IS the direction.
        const maxUpMove =
          Math.max(...windowCandles.map((c) => c.high)) - startP;
        const maxDownMove =
          startP - Math.min(...windowCandles.map((c) => c.low));
        const goingUp = maxUpMove > maxDownMove;
        let extremeCandle = windowCandles[0];
        for (const c of windowCandles) {
          if (goingUp ? c.high > extremeCandle.high : c.low < extremeCandle.low)
            extremeCandle = c;
        }
        const direction = goingUp ? "up" : "down";
        cascadeEndMs = findCandleConfirmedEndMs(
          extremeCandle.openTimeMs,
          direction,
          exhaustionEndMs,
        );
      }
    ...
    displayedCascadeCount++;
    console.log("-".repeat(118));
    console.log(
      `CASCADE #${displayedCascadeCount}: ${isoUtc(cascadeStartMs)} -> ${isoUtc(cascadeEndMs)}  (${((cascadeEndMs - cascadeStartMs) / 60000).toFixed(1)}m, ${participating.length}/${SYMBOLS.length} symbols, total liq ${fmtUsd(totalCascadeUsd)})`,
    );
    console.log(
      `  End refined from liquidation-exhaustion (${isoUtc(exhaustionEndMs)}) to BTC candle-confirmed end (${isoUtc(cascadeEndMs)}) -- 1m/3m/5m all agree BTC stopped extending.`,
    );
    console.log("-".repeat(118));
    ...
    for (const r of records) {
      ...
      const atrPctOfPrice =
        r.atrAtEnd !== null && r.endPrice
          ? (r.atrAtEnd / r.endPrice) * 100
          : null;
      const dynamicThreshold =
        atrPctOfPrice !== null
          ? DYNAMIC_THRESHOLD_ATR_MULTIPLIER * atrPctOfPrice
          : null;
      const classification =
        dynamicThreshold === null
          ? "UNKNOWN"
          : r.normalizedForward[2] > dynamicThreshold
            ? "REVERSAL"
            : r.normalizedForward[2] < -dynamicThreshold
              ? "CONTINUATION"
              : "FLAT";
      ...
      globalCoinCascadeRows.push({
        cascadeIdx: displayedCascadeCount,
        symbol: r.symbol,
        classification,
  ...
  console.log(
    `Classification threshold is DYNAMIC: +-${DYNAMIC_THRESHOLD_ATR_MULTIPLIER}x that coin's OWN ATR (as % of its own price at cascade end) -- not a fixed number across all 10 symbols.`,
  );
  console.log("=".repeat(118));
  ...
  printClassGroup("UNKNOWN"); // no ATR available for this coin at cascade end -- shown for transparency, not silently dropped
  ...
  console.log(`\n${"=".repeat(118)}`);
  console.log(
    `TOTAL: ${displayedCascadeCount} BIG cascade episode(s) (of ${mergedRuns.length} raw candidates) in ${days} day(s) at >= ${minDistinctSymbols}/${SYMBOLS.length} symbol threshold, total liq >= ${fmtUsd(minTotalCascadeUsd)}.`,
  );
  console.log(
