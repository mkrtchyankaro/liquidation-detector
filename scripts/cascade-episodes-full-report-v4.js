...
//
//   node scripts/cascade-episodes-full-report-v4.js 7 9
//
// v4 CHANGES (operator-requested):
...
async function main() {
  ...
  for (let epIdx = 0; epIdx < runs.length; epIdx++) {
    const [startBucket, coreEndBucket] = runs[epIdx];
    const exhaustionEndBucket = findExhaustionEndBucket(
      startBucket,
      coreEndBucket,
    );
