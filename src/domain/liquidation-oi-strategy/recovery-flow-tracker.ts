...
import {
  cvdUsd,
  imbalancePct,
  classifySpotConfirmation,
  classifyRecoveryOiMove,
  type SpotConfirmationLabel,
  type RecoveryOiMoveLabel,
} from "./order-flow-interpretation";
...
const MAX_BUFFER_MS = 15 * 60_000; // Sep 19 2026 (Karo), operator-reported PRODUCTION MEMORY INCIDENT -- reduced from 30min; Recovery Flow windows are consistently short (minutes, not tens of minutes -- see the operator's own example durations "74s"/"68s"), so 15min retains a large safety margin while roughly halving this tracker's own memory footprint.
...
export class RecoveryFlowTracker {
  ...
  onLifecycleTransition(
    symbol: string,
    before: RecoveryLifecycleSnapshot | null,
    after: RecoveryLifecycleSnapshot | null,
    nowMs: number,
  ): void {
    ...
    if (
      existing !== undefined &&
      (after === null || after.episodeId !== existing.episodeId)
    ) {
    ...
    if (after !== null && this.state.get(symbol) === undefined) {
      this.state.set(symbol, {
        ...
        symbol,
        victim: after.victim,
        recoveryExtremePrice: after.episodeMaxAdverseExtreme,
        recoveryExtremeAt: nowMs,
        futuresBuffer: [],
    ...
    if (
      ...
      s.frozenSnapshot = this.buildSnapshot(
        s,
        after.episodeEndPrice ?? after.episodeMaxAdverseExtreme,
        after.episodeEndTime ?? nowMs,
        nowMs,
      );
    }
  }
  ...
  private buildSnapshot(
    s: SymbolRecoveryState,
    confirmationPrice: number,
    recoveryEndAt: number,
    frozenAtMs: number,
  ): RecoveryFlowFrozenStats {
    ...
    const inWindow = (e: TradeBufferEntry): boolean =>
      e.ts >= recoveryStartAt && e.ts <= recoveryEndAt;

    const futuresInWindow = s.futuresBuffer.filter(inWindow);
    const futuresBuyUsd = futuresInWindow
      .filter((e) => e.aggressor === "BUY")
      .reduce((a, e) => a + e.quoteQty, 0);
    const futuresSellUsd = futuresInWindow
      .filter((e) => e.aggressor === "SELL")
      .reduce((a, e) => a + e.quoteQty, 0);
    const futuresPriceDeltaPct =
      s.recoveryExtremePrice > 0
        ? ((confirmationPrice - s.recoveryExtremePrice) /
            s.recoveryExtremePrice) *
          100
        : 0;

    const spotInWindow = s.spotBuffer.filter(inWindow);
    const spotDataAvailable = spotInWindow.length > 0;
    const spotBuyUsd = spotInWindow
      .filter((e) => e.aggressor === "BUY")
      .reduce((a, e) => a + e.quoteQty, 0);
    const spotSellUsd = spotInWindow
      .filter((e) => e.aggressor === "SELL")
      .reduce((a, e) => a + e.quoteQty, 0);
    const spotVolumeUsd = spotBuyUsd + spotSellUsd;
    const spotImbalance = spotDataAvailable
      ? imbalancePct(spotBuyUsd, spotSellUsd)
      : 0;
    const spotPriceStart = spotDataAvailable ? spotInWindow[0]!.price : null;
    const spotPriceEnd = spotDataAvailable
      ? spotInWindow[spotInWindow.length - 1]!.price
      : null;
    const spotPriceDeltaPct =
    ...
    const spotConfirmationLabel = classifySpotConfirmation(
      candidateSide,
      spotImbalance,
      this.config.spotImbalanceNeutralBandPct,
      spotDataAvailable,
    );
    const recoveryOiMoveLabel = classifyRecoveryOiMove(
      candidateSide,
      oiDeltaPct,
      this.config.oiFlatBandPct,
    );
    ...
    const snapshot: RecoveryFlowFrozenStats = {
      ...
      recoveryEndAt,
      recoveryDurationMs,
      recoveryExtremePrice: s.recoveryExtremePrice,
      recoveryConfirmationPrice: confirmationPrice,
      recoveryMoveAtr: null,
      spotDataAvailable,
      recoverySpotTakerBuyUsd: spotBuyUsd,
      recoverySpotTakerSellUsd: spotSellUsd,
      recoverySpotCvdUsd: cvdUsd(spotBuyUsd, spotSellUsd),
      recoverySpotImbalancePct: spotImbalance,
      recoverySpotPriceDeltaPct: spotPriceDeltaPct,
      recoverySpotVolumeUsd: spotVolumeUsd,
      spotConfirmationLabel,
      recoveryFuturesTakerBuyUsd: futuresBuyUsd,
      recoveryFuturesTakerSellUsd: futuresSellUsd,
      recoveryFuturesCvdUsd: cvdUsd(futuresBuyUsd, futuresSellUsd),
      recoveryFuturesImbalancePct: imbalancePct(futuresBuyUsd, futuresSellUsd),
      recoveryFuturesPriceDeltaPct: futuresPriceDeltaPct,
      oiDataAvailable,
      recoveryOiStart: oiStart,
      recoveryOiEnd: oiEnd,
      recoveryOiDelta: oiDelta,
      recoveryOiDeltaPct: oiDeltaPct,
      recoveryOiMoveLabel,
      frozenAtMs,
    };
    ...
    log.info(
      { ...snapshot },
      `[RECOVERY_FLOW_FROZEN] ${s.symbol} episodeId=${s.episodeId}`,
    );
    return snapshot;
  }
}
