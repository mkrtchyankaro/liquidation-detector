...
const MAX_BUFFER_MS = 45 * 60_000; // Sep 19 2026 (Karo), operator-reported PRODUCTION MEMORY INCIDENT -- reduced from 90min (which, combined with RecoveryFlowTracker's own separate 30min buffers for the SAME underlying Spot+Futures aggTrade streams, was implicated in repeated pm2 max_memory_restart (600M) kills, wiping in-progress episode state before it could reach WAIT). 45min still comfortably covers the large majority of observed episode durations (historical episode list showed most resolving well under an hour); very long-tailed episodes lose only their EARLIEST Flush Flow trade coverage -- the event chain, extremes, OI, and Recovery Flow remain fully correct regardless.
...
export class EpisodeResearchRecorder {
  ...
  private nearestOi(
    ...
    return {
      oiTimestamp: best.fetchedAt,
      oiAgeMs: bestDelta,
      oiValue: best.contracts,
      oiNotionalUsd: null,
    };
  }
  ...
  private pointSnapshot(
    ...
    ts: number,
    price: number,
  ): { market: MarketPointSnapshot; oi: OiPointSnapshot | null } {
    const s = this.state.get(symbol);
    const market: MarketPointSnapshot = {
      ts,
      ...this.market.getBasisSnapshot(symbol, ts),
    };
    const oiRaw = s !== undefined ? this.nearestOi(s.oiBuffer, ts) : null;
    const oi =
      oiRaw !== null
        ? { ...oiRaw, oiNotionalUsd: oiRaw.oiValue * price }
        : null;
    return { market, oi };
  }
  ...
  onLifecycleTransition(
    symbol: string,
    before: ResearchLifecycleSnapshot | null,
    after: ResearchLifecycleSnapshot | null,
    nowMs: number,
  ): void {
    if (
      after !== null &&
      (before === null || before.episodeId !== after.episodeId) &&
      !this.state.has(symbol)
    ) {
      const startSnap = this.pointSnapshot(
        symbol,
        nowMs,
        after.episodeStartPrice,
      );
      const record: EpisodeResearchRecord = {
        ...
        episodeStartSnapshot: {
          ts: nowMs,
          price: after.episodeStartPrice,
          ...startSnap,
        },
        liquidationEventSnapshots: [],
        extremeSnapshots: [],
        finalExtremeSnapshot: null,
        lastLiquidationEventSnapshot: null,
        episodeEndSnapshot: null,
        entrySnapshot: null,
        endReason: null,
        flushFlow: null,
        recoveryFlow: null,
      ...
      this.state.set(symbol, {
        record,
        extremeAt: nowMs,
        extremePrice: after.episodeStartPrice,
        recoveryFrozen: false,
        futuresBuffer: [],
        futuresSeenIds: new Set(),
        spotBuffer: [],
        spotSeenIds: new Set(),
        oiBuffer: [],
      });
    ...
    if (after.episodeMaxAdverseExtreme !== s.extremePrice) {
      ...
      const snap = this.pointSnapshot(
        symbol,
        nowMs,
        after.episodeMaxAdverseExtreme,
      );
      const extremeSnapshot: ExtremeSnapshot = {
        ts: nowMs,
        price: after.episodeMaxAdverseExtreme,
        ...snap,
      };
  ...
  onEntry(
    ...
    log.info(
      {
        episodeId: s.record.episodeId,
        symbol,
        entryReason,
        eventCount: s.record.liquidationEventSnapshots.length,
        extremeCount: s.record.extremeSnapshots.length,
      },
      `[EPISODE_RESEARCH_ENTRY] ${symbol} episodeId=${s.record.episodeId}`,
    );
  }
  ...
  onEpisodeTerminal(
    ...
    if (s.record.entrySnapshot === null) {
      ...
      const endSnap = s.record.episodeEndSnapshot ?? {
        ts: endTs,
        price: endPrice,
        ...this.pointSnapshot(symbol, endTs, endPrice),
      };
    ...
    log.info(
      {
        ...
        symbol,
        reason,
        noEntry: s.record.entrySnapshot === null,
        eventCount: s.record.liquidationEventSnapshots.length,
        extremeCount: s.record.extremeSnapshots.length,
      },
      `[EPISODE_RESEARCH_TERMINAL] ${symbol} episodeId=${s.record.episodeId}`,
    );
    return s.record;
  }
  ...
  private computeFlushFlow(s: SymbolResearchState): void {
    ...
    const flush = this.buildFlowWindow(
      s,
      startAt,
      endAt,
      s.record.episodeStartSnapshot.price,
      s.extremePrice,
      s.record.liquidationEventSnapshots.reduce((a, e) => a + e.notionalUsd, 0),
      s.record.liquidationEventSnapshots.length,
      s.record.extremeSnapshots.length,
    );
    s.record.flushFlow = flush;
  }

  private computeRecoveryFlow(
    s: SymbolResearchState,
    endAt: number,
    endPrice: number,
    freeze: boolean,
  ): void {
    ...
    const recovery = this.buildFlowWindow(
      s,
      startAt,
      endAt,
      s.extremePrice,
    ...
    if (
      s.record.recoveryFlow !== null &&
      s.record.recoveryFlow.startAt !== startAt
    ) {
  ...
  private buildFlowWindow(
    ...
    const inWindow = (e: TradeBufferEntry): boolean =>
      e.ts >= startAt && e.ts <= endAt;
    const futuresIn = s.futuresBuffer.filter(inWindow);
    const futuresBuyUsd = futuresIn
      .filter((e) => e.aggressor === "BUY")
      .reduce((a, e) => a + e.quoteQty, 0);
    const futuresSellUsd = futuresIn
      .filter((e) => e.aggressor === "SELL")
      .reduce((a, e) => a + e.quoteQty, 0);

    const spotIn = s.spotBuffer.filter(inWindow);
    const spotDataAvailable = spotIn.length > 0;
    const spotBuyUsd = spotIn
      .filter((e) => e.aggressor === "BUY")
      .reduce((a, e) => a + e.quoteQty, 0);
    const spotSellUsd = spotIn
      .filter((e) => e.aggressor === "SELL")
      .reduce((a, e) => a + e.quoteQty, 0);
    const spotPriceStart = spotDataAvailable ? spotIn[0]!.price : null;
    const spotPriceEnd = spotDataAvailable
      ? spotIn[spotIn.length - 1]!.price
      : null;

    const oiStart = this.nearestOi(s.oiBuffer, startAt);
    const oiEnd = this.nearestOi(s.oiBuffer, endAt);
    const oiDeltaUsd =
      oiStart !== null && oiEnd !== null
        ? (oiEnd.oiValue - oiStart.oiValue) * priceEnd
        : null;
    const oiDeltaPct =
      oiStart !== null && oiEnd !== null && oiStart.oiValue !== 0
        ? ((oiEnd.oiValue - oiStart.oiValue) / oiStart.oiValue) * 100
        : null;
    ...
    return {
      ...
      futuresBuyUsd,
      futuresSellUsd,
      futuresDelta: cvdUsd(futuresBuyUsd, futuresSellUsd),
      futuresBuySellRatio:
        futuresSellUsd > 0 ? futuresBuyUsd / futuresSellUsd : null,
      spotBuyUsd,
      spotSellUsd,
      spotDelta: cvdUsd(spotBuyUsd, spotSellUsd),
      ...
      spotPriceEnd,
      spotPriceChangePct:
        spotPriceStart !== null && spotPriceEnd !== null && spotPriceStart > 0
          ? ((spotPriceEnd - spotPriceStart) / spotPriceStart) * 100
          : null,
      futuresPriceStart: priceStart,
      futuresPriceEnd: priceEnd,
      futuresPriceChangePct:
        priceStart > 0 ? ((priceEnd - priceStart) / priceStart) * 100 : 0,
      basisStartBps: basisStart.basisBps,
      basisEndBps: basisEnd.basisBps,
      basisChangeBps:
        basisStart.basisBps !== null && basisEnd.basisBps !== null
          ? basisEnd.basisBps - basisStart.basisBps
          : null,
  ...
  private computeBasisRecovery(
    s: SymbolResearchState,
    endSnap: { price: number; market: MarketPointSnapshot },
  ): void {
    const extremeMarket = s.record.finalExtremeSnapshot?.market ?? null;
    if (
      extremeMarket === null ||
      extremeMarket.basisUsd === null ||
      endSnap.market.basisUsd === null ||
      extremeMarket.futuresMid === null ||
      extremeMarket.spotMid === null ||
      endSnap.market.futuresMid === null ||
      endSnap.market.spotMid === null
    ) {
      s.record.basisRecovery = {
        basisAtExtremeUsd: extremeMarket?.basisUsd ?? null,
        basisAtExtremeBps: extremeMarket?.basisBps ?? null,
        basisAtEntryOrEndUsd: endSnap.market.basisUsd,
        basisAtEntryOrEndBps: endSnap.market.basisBps,
        basisClosedUsd: null,
        basisClosedPct: null,
        futuresPriceContributionUsd: null,
        spotPriceContributionUsd: null,
        futuresContributionPct: null,
        spotContributionPct: null,
        spotHeldAfterExtreme: null,
        spotMadeNewExtreme: null,
        futuresMadeRecovery: null,
    ...
    const basisClosedPct =
      basisAtExtremeUsd !== 0
        ? (basisClosedUsd / Math.abs(basisAtExtremeUsd)) * 100
        : null;

    const futuresPriceContributionUsd =
      endSnap.market.futuresMid - extremeMarket.futuresMid;
    const spotPriceContributionUsd = -(
      endSnap.market.spotMid - extremeMarket.spotMid
    );
    const totalContribution =
      Math.abs(futuresPriceContributionUsd) +
      Math.abs(spotPriceContributionUsd);
    const futuresContributionPct =
      totalContribution > 0
        ? (Math.abs(futuresPriceContributionUsd) / totalContribution) * 100
        : null;
    const spotContributionPct =
      totalContribution > 0
        ? (Math.abs(spotPriceContributionUsd) / totalContribution) * 100
        : null;

    const victim = s.record.victim;
    const futuresMadeRecovery =
      victim === "LONG"
        ? endSnap.market.futuresMid > extremeMarket.futuresMid
        : endSnap.market.futuresMid < extremeMarket.futuresMid;
    const spotMovedAdverse =
      victim === "LONG"
        ? endSnap.market.spotMid < extremeMarket.spotMid
        : endSnap.market.spotMid > extremeMarket.spotMid;
    ...
    if (!futuresMoved && spotMovedFavorably)
      convergenceDirection = "SPOT_TOWARD_FUTURES";
    else if (futuresMoved && !spotMovedFavorably)
      convergenceDirection = "FUTURES_TOWARD_SPOT";
    ...
    s.record.basisRecovery = {
      basisAtExtremeUsd,
      basisAtExtremeBps: extremeMarket.basisBps,
      basisAtEntryOrEndUsd: basisAtEndUsd,
      basisAtEntryOrEndBps: endSnap.market.basisBps,
      basisClosedUsd,
      basisClosedPct,
      futuresPriceContributionUsd,
      spotPriceContributionUsd,
      futuresContributionPct,
      spotContributionPct,
      spotHeldAfterExtreme,
      spotMadeNewExtreme,
      futuresMadeRecovery,
