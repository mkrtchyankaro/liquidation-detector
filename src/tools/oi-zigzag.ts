...
const LABEL: Record<Wave["kind"], string> = {
  LONG_CLEANING: "CLEAN LONG ",
  SHORT_CLEANING: "CLEAN SHORT",
  OI_DOWN: "oi down    ",
  OI_UP: "OI UP      ",
};
...
async function main(): Promise<void> {
  const env = loadEnv();
  const symbols = arg("symbols", "ETH,SOL,ADA")
    .split(",")
  ...
  const slMode = (
    slModeArg === "PCT" || slModeArg === "STRUCTURE" ? slModeArg : "TARGET"
  ) as "TARGET" | "PCT" | "STRUCTURE";
  const slPct = Number(arg("sl", String(DEFAULT_CHAIN_PARAMS.slPct))),
    tpPct = Number(arg("tp", String(DEFAULT_CHAIN_PARAMS.tpPct)));
  const rr = Number(arg("rr", String(DEFAULT_CHAIN_PARAMS.rr))),
    atrBuffer = Number(arg("atr", String(DEFAULT_CHAIN_PARAMS.atrBuffer)));
  const maxConfirmDelayMin = Number(
    arg("maxdelay", String(DEFAULT_CHAIN_PARAMS.maxConfirmDelayMin)),
  );
  const tpShare = Number(arg("tpshare", String(DEFAULT_CHAIN_PARAMS.tpShare)));
  const depthMode = (
    arg("depth", "cleaning").toUpperCase() === "CALIBRATED"
      ? "CALIBRATED"
      : "CLEANING"
  ) as "CLEANING" | "CALIBRATED";
  ...
  const exitLabel =
    slMode === "PCT"
      ? `SL ${slPct}% / TP ${tpPct}%`
      : slMode === "STRUCTURE"
        ? `SL = last extreme + ${atrBuffer} ATR (min ${DEFAULT_CHAIN_PARAMS.minSlPct}%), TP = ${rr}R`
    ...
    for (const symbol of symbols) {
      ...
      const bars = dense(
        rows.map((r) => ({
          ...
          close: r.close,
          oi: r.oiLast,
          longLiq: r.longLiqUsd ?? 0,
          shortLiq: r.shortLiqUsd ?? 0,
        })),
      );
      if (bars.length < 120) {
        console.log(
          `\n${symbol}: not enough minute bars (${bars.length}) -- run minute-bars-backfill first`,
        );
        continue;
      }
      ...
      console.log(
        `WAVE         FROM         TO           MIN   OI ${coin.padEnd(5)}         PRICE start -> end   (low / high)            LIQ long / short`,
      );
      for (const w of waves) {
        console.log(
          `${LABEL[w.kind]}  ${stamp(w.from.ts)}  ${stamp(w.to.ts)}  ${String(w.minutes).padStart(4)}  ${(w.oiEnd >= w.oiStart ? "+" : "-") + coins(w.coins)} (${w.oiChangePct >= 0 ? "+" : ""}${w.oiChangePct.toFixed(2)}%)`.padEnd(
            70,
          ) +
            `${px(w.priceStart)} -> ${px(w.priceEnd)}  (${px(w.priceLow)} / ${px(w.priceHigh)})`.padEnd(
              42,
            ) +
            `${usd(w.longLiqUsd)} / ${usd(w.shortLiqUsd)}${w.confirmed ? "" : "   <- still running"}`,
        );
      }
      console.log(
      ...
      if (html) {
        ...
        const trades = chains
          .filter((c) => c.trade?.result)
          .map((c) => ({ ...c.trade!, grade: c.quality.grade }));
  ...
  summary(
    allChains,
    `${exitLabel} · depth ${depthMode}${depthMode === "CALIBRATED" ? " (last 24h waves, ATR)" : " (this cleaning)"}`,
  );
  summary(
    otherChains,
    `${exitLabel} · depth ${otherMode}${otherMode === "CALIBRATED" ? " (last 24h waves, ATR)" : " (this cleaning)"}  [comparison]`,
  );
}

function summary(all: Chain[], exitLabel: string): void {
  const med = (v: number[]): string => {
    const s = [...v].sort((a, b) => a - b);
    return s.length ? s[s.length >> 1].toFixed(2) : "n/a";
  };
  console.log(
    `\n===== SUMMARY by grade (A strongest) -- late entry: ${exitLabel} =====`,
  );
  console.log(
    "GRADE  CLEANINGS  real/expected (median; 1.00 = exact)   TRADES  TP  SL  open  win%    netR",
  );
  for (const g of ["A", "B", "C"] as const) {
    const cs = all.filter((c) => c.quality.grade === g);
    const done = cs.filter(
      (c) => c.expectedMove && c.actualUp !== null && c.resolution?.confirmed,
    );
    const ratios = done.map(
      (c) => Math.max(c.actualUp!, c.actualDown!) / c.expectedMove!,
    );
    const tr = cs.map((c) => c.trade).filter((t) => t && t.result);
    const tp = tr.filter((t) => t!.result === "TP").length,
      sl = tr.filter((t) => t!.result === "SL").length,
      open = tr.filter((t) => t!.result === "OPEN").length;
    const net = tr.reduce((a, t) => a + (t!.netR ?? 0), 0);
    console.log(
      `  ${g}    ${String(cs.length).padStart(5)}      ${med(ratios).padStart(6)}  (n=${done.length})                    ${String(tr.length).padStart(4)}  ${String(tp).padStart(2)}  ${String(sl).padStart(2)}  ${String(open).padStart(3)}  ${tp + sl ? ((100 * tp) / (tp + sl)).toFixed(0).padStart(3) : "n/a"}%  ${net.toFixed(2).padStart(6)}`,
    );
  }
  console.log(
    "netR after fees (taker entry, maker TP / taker SL). Tiny sample -- a direction to look, not proof.",
  );
}
...
function printChain(c: Chain, coin: string): void {
  const w = c.cleaning,
    a = c.accumulation,
    r = c.resolution;
  const pct = (m: number, base: number): string =>
    `${((100 * m) / base).toFixed(2)}%`;
  const q = c.quality;
  console.log(
  ...
  console.log(
    `   closed ${coins(w.coins)} ${coin}   price moved ${px(c.cleaningMove)} (${pct(c.cleaningMove, w.priceStart)})   liq ${usd(w.longLiqUsd)} long / ${usd(w.shortLiqUsd)} short`,
  );
  console.log(
    c.depthPer1k >= 0.01
      ? `   depth: ${px(c.depthPer1k)} per 1,000 ${coin}`
      : `   depth: ${px(c.depthPer1k * 1000)} per 1,000,000 ${coin}`,
  );
  if (!a) {
    console.log("   accumulation: not yet");
    return;
  }
  console.log(
    `   accumulation ${stamp(a.from.ts)} -> ${stamp(a.to.ts)} (${a.minutes}m${a.confirmed ? `, OI top seen at ${stamp(a.to.confirmedTs)}` : ", still running"})  opened ${coins(a.coins)} ${coin}  zone ${px(a.priceLow)} - ${px(a.priceHigh)}  ends at ${px(a.priceEnd)}`,
  );
  console.log(
    `   EXPECTED move: ${px(c.expectedMove!)} (${pct(c.expectedMove!, a.priceEnd)})${c.calibration ? `  [calibrated: ${c.calibration.atrPerOiPct.toFixed(2)} ATR per 1% OI, from ${c.calibration.waves} waves of the last 24h]` : ""}`,
  );
  if (!r) {
    printTrade(c);
    console.log("   resolution: not yet");
    return;
  }
  printTrade(c);
  console.log(
    `   REAL next wave ${stamp(r.from.ts)} -> ${stamp(r.to.ts)} (${r.minutes}m${r.confirmed ? "" : ", still running"}): up ${px(c.actualUp!)} (${pct(c.actualUp!, a.priceEnd)}) / down ${px(c.actualDown!)} (${pct(c.actualDown!, a.priceEnd)})  -> closed ${coins(r.coins)} ${coin}`,
  );
}
...
function tradeTable(symbol: string, chains: Chain[]): void {
  ...
  console.log(
    "GR SIDE  CLEANING (start -> OI bottom)   ENTRY time    price       SL (dist)              TP (dist)              RESULT  EXIT time     held   netR",
  );
  for (const c of rows) {
    const t = c.trade!,
      w = c.cleaning;
    const dist = (p: number): string =>
      `${((100 * Math.abs(p - t.entry)) / t.entry).toFixed(2)}%`;
    ...
    if (t.skipReason) {
      console.log(
        `${head}  no trade: ${t.skipReason} (already moved ${dist(t.entry + t.alreadyMoved)}, remaining ${((100 * t.remaining) / t.entry).toFixed(2)}%)`,
      );
      continue;
    }
    console.log(
      `${head}  ${`${px(t.slPrice!)} (${dist(t.slPrice!)})`.padEnd(21)}  ${`${px(t.tpPrice!)} (${dist(t.tpPrice!)})`.padEnd(21)}  ${t.result!.padEnd(6)}  ${t.exitTs ? stamp(t.exitTs) : "-".padEnd(11)}  ${(t.minutes !== null ? `${t.minutes}m` : "-").padStart(5)}  ${t.netR!.toFixed(2).padStart(5)}`,
    );
  }
}
...
function printTrade(c: Chain): void {
  ...
  if (t.skipReason) {
    console.log(`${head}  -> NO TRADE (${t.skipReason})`);
    return;
  }
  console.log(
    `${head}  -> ${t.side === "LONG" ? "BUY" : "SELL"}: ${t.result}${t.minutes !== null ? ` in ${t.minutes}m` : ""}  netR ${t.netR!.toFixed(2)}`,
  );
}
