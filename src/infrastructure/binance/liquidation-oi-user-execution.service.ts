...
async function failSafeMarketClose(rest: BinanceRestLike, symbol: string, closeSide: "BUY" | "SELL", quantity: number, filters: SymbolFilters): Promise<void> {
  try {
    ...
    log.error({ symbol, err: err instanceof Error ? err.message : String(err) }, "[LOX_FAILSAFE_CLOSE_FAILED] -- CRITICAL: position may remain open and unprotected, requires immediate manual attention");
  }
}
