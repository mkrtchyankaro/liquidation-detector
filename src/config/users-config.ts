...
export function parseAppConfig(
  ...
  if (
    r.realOrdersEnabled !== undefined &&
    typeof r.realOrdersEnabled !== "boolean"
  )
  ...
  const users: UserConfig[] = (r.users as Array<Record<string, unknown>>).map(
    (u, i) => {
      const userId =
        typeof u.userId === "string" ? u.userId.trim().toLowerCase() : "";
      if (!USER_ID.test(userId))
        fail(
          path,
          `users[${i}].userId "${String(u.userId)}" must be 2-32 chars: a-z, 0-9, _ or -, starting with a letter`,
        );
      ...
      if (t && t.enabled === true) {
        const chatIds = (
          Array.isArray(t.chatIds)
            ? t.chatIds.map(String)
            : String(t.chatIds ?? "").split(",")
        )
          .map((c) => c.trim())
          .filter(Boolean);
        if (
          typeof t.botToken !== "string" ||
          !t.botToken ||
          chatIds.length === 0
        )
          fail(
            path,
            `${where}: telegram.enabled=true needs botToken and chatIds`,
          );
      ...
      if (b && b.enabled === true) {
        if (
          ...
          !b.apiKey ||
          typeof b.apiSecret !== "string" ||
          !b.apiSecret
        )
          fail(
            path,
            `${where}: binance.enabled=true needs apiKey and apiSecret`,
          );
        ...
        if (
          ...
          fail(path, `${where}: binance.leverage must be an integer 1..125`);
        const marginMode = b.marginMode ?? "ISOLATED";
        if (marginMode !== "ISOLATED" && marginMode !== "CROSSED")
          fail(
            path,
            `${where}: binance.marginMode must be "ISOLATED" or "CROSSED"`,
          );
        binance = {
          apiKey: b.apiKey,
      ...
      if (typeof riskUsd !== "number" || !(riskUsd > 0))
        fail(path, `${where}: risk.riskUsd must be a positive number`);
...
export function loadAppConfig(
  ...
  if (!fs.existsSync(path))
    throw new Error(
      `users config not found at ${path} -- copy users.config.example.json and fill in real values`,
    );
