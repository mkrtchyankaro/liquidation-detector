...
export class V9MinuteStore {
  ...
  prune(keepFromTs: number): void {
    ...
    for (const map of [this.liq, this.oi, this.prices] as Array<
      Map<number, unknown>
    >) {
  ...
  toBuckets(from: number, until: number): Bucket[] {
    ...
    for (let ts = minuteOf(from); ts <= minuteOf(until); ts += MINUTE_MS) {
      ...
      const inRange =
        o !== undefined && o.updated >= from && o.updated <= until;
      if (inRange) {
        lastOi = o!.oi;
        lastPrice = o!.price;
      }
