...
export function typicalMinuteRangeBefore(
  polls: readonly Poll[],
  ts: number,
): number {
  ...
  for (
    let i = lowerBound(polls, from);
    i < polls.length && polls[i].ts < ts;
    i++
  ) {
    ...
    if (!r) byMinute.set(m, { lo: polls[i].price, hi: polls[i].price });
      ...
      r.lo = Math.min(r.lo, polls[i].price);
      r.hi = Math.max(r.hi, polls[i].price);
    }
  }
...
export function stopForVariant(
  v: SlVariant,
  side: "LONG" | "SHORT",
  entry: number,
  currentStop: number,
  ...
  switch (v) {
    case "CURRENT":
      return currentStop;
    case "MIN_0.33%":
      return entry + dir * Math.max(risk, entry * 0.0033); // stop-out fees <= ~0.3R
    case "PLUS_1_MINUTE_RANGE":
      return currentStop + dir * minuteRange;
    case "X1.5":
      return entry + dir * risk * 1.5;
    case "X2":
      return entry + dir * risk * 2;
  }
}
