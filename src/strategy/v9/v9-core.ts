...
export function buildBuckets(
  liquidations: readonly LiqEvent[],
  ...
  for (const event of liquidations) {
    ...
    if (!bucket) continue;
    if (event.victim === "LONG") bucket.long += event.usd;
    else bucket.short += event.usd;
    bucket.count++;
  }
...
export function usableRange(buckets: readonly Bucket[]): Bucket[] | null {
  ...
  for (let i = 0; i < buckets.length; i++)
    if (Number.isFinite(buckets[i].oi)) {
      if (first < 0) first = i;
      last = i;
    }
...
function linearCost(
  y: readonly number[],
): (a: number, b: number) => { sse: number; slope: number } {
...
const sideOf = (long: number, short: number): Victim =>
...
export function subEpisodes(
  ...
  const close = (
    ...
    let long = 0,
      short = 0,
      count = 0,
      minimum = buckets[Math.max(0, start - 1)].oi;
    const baseline = minimum;
    for (let i = start; i < stop; i++) {
      long += buckets[i].long;
      short += buckets[i].short;
      count += buckets[i].count;
      minimum = Math.min(minimum, buckets[i].oi);
    }
    const last = buckets[stop - 1];
    const endTs =
      stop < buckets.length ? buckets[stop].ts : last.ts + MINUTE_MS;
    if (count)
      out.push({
        ...
        sIdx: start,
        eIdx: stop,
        victim: active.side ?? sideOf(long, short),
        long,
        short,
        count,
        oiDropPct:
  ...
  if (active)
    close(
      buckets.length,
      (active as { growthStart: number }).growthStart >= 0
        ? "GROWING_AT_END"
        : "OPEN_AT_DATA_END",
      true,
    );
...
export interface Episode {
  ...
  endReason: string;
  parts: number;
  partRanges: Array<[number, number]>;
  rightCensored: boolean;
}
...
export function mergeEpisodes(
  ...
  for (const sub of subs) {
    ...
    if (sub.victim === side) group.push(sub);
      ...
      finish(
        "OPPOSITE_EPISODE",
        k < sub.eIdx ? buckets[k].ts + MINUTE_MS : NaN,
      );
      group = [sub];
    }
...
export function episodeFeatures(
  buckets: readonly Bucket[],
  e: Episode,
): EpisodeFeatures {
  ...
  for (let i = e.sIdx; i < e.eIdx; i++) {
    victimLiq += victimOf(buckets[i]);
    oppLiq += oppOf(buckets[i]);
  }
  const dom = e.partRanges.every(([a, z]) => {
    let v = 0,
      o = 0;
    for (let i = a; i < z; i++) {
      v += victimOf(buckets[i]);
      o += oppOf(buckets[i]);
    }
    return v > o;
  });
  ...
  const segment = (
    a: number,
    z: number,
  ): { move: number; drop: number; eff: number } => {
...
export function buildReference(
  dirFeatures: ReadonlyArray<Pick<EpisodeFeatures, "clr" | "dirMove">>,
): SelectionReference {
  return {
    medianClr: median(dirFeatures.map((f) => f.clr)),
    medianMove: median(dirFeatures.map((f) => f.dirMove)),
    sampleCount: dirFeatures.length,
  };
}
...
export function selectEpisode(
  ...
  const checks = {
    DOM: f.dom,
    DIR: f.dir,
    CLR: f.clr > ref.medianClr,
    MOV: f.dirMove >= ref.medianMove,
    EXH: f.exh,
  };
...
export function analyzeWindow(
  ...
  from: number,
  until: number,
): { buckets: Bucket[]; episodes: Episode[] } | null {
  // Same input filtering as the research loader.
  const events = liquidations.filter(
    (x) =>
      x.ts >= from && x.ts <= until && Number.isFinite(x.usd) && x.usd >= 0,
  );
  const observations = oi.filter(
    (x) =>
  ...
  return {
    buckets: usable,
    episodes: mergeEpisodes(usable, subEpisodes(usable, regimes, until)),
  };
}
