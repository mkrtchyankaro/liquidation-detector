...
export interface V9EngineSettings {
  ...
  /** OI REGROWTH (Johnny, Sep 25 2026) -- no time windows: at the signal,
   *  look back. The cleaning closed D coins (episode start OI -> its OI
   *  bottom). Between that bottom and the OI turn (where the confirming drop
   *  started) OI grew by G coins. Trade only if G >= this share of D
   *  (0.5 = half of the closed positions were re-opened). null = off. */
  minRegrowShare: number | null;
}
...
export function regrowShare(buckets: readonly Bucket[], e: Episode): number | null {
  ...
  return Math.max(0, turnOi - low) / drop;
}
