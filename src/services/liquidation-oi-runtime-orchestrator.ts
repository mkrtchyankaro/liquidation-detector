...
export class LiquidationOiRuntimeOrchestrator {
  ...
  private async handleEntryReady(symbol: string, ownershipId: string, episode: { victim: Side; extremePrice: number; sameDirectionLiqUsd: number; startOiQuantity: number | null; minOiQuantity: number | null; currentOiQuantity: number | null }, watchResult: WatchQualificationResult | null, entryPrice: number, atr3m: number, nowMs: number, orderBook: OrderBookObservation | null, counterMoveAtr: number, distanceFromExtremeAtr: number, episodeEndOiQuantity: number | null, capacityAtr: number | null, candidateTpPrice: number | null, candidateSlPrice: number | null, netRR: number | null, postEndOiCreationUsd: number | null, oiToLiqRatio: number | null, episodeEndPrice: number | null, episodeEndTime: number | null): Promise<void> {
    ...
    const emergencyHardStopPrice = candidateSide === "LONG"
      ? strategyInvalidationPrice - atr3m * this.strategyConfig.emergencyHardStopBufferAtrMultiple
      : strategyInvalidationPrice + atr3m * this.strategyConfig.emergencyHardStopBufferAtrMultiple;
    ...
    await this.globalSignalRepo.upsertSignal({
      ...
      extremePrice: episode.extremePrice, entryPrice, strategyInvalidationPrice, emergencyHardStopPrice,
    ...
    for (const runtime of this.getUserRuntimes()) {
      try {
        outcomes.push(await this.executeForUser(runtime, symbol, globalSignalId, candidateSide, entryPrice, strategyInvalidationPrice, emergencyHardStopPrice, tpPrice, watchResult.episodePercentileRank, episode.sameDirectionLiqUsd, counterMoveAtr, capacityAtr, netRR, orderBook, oiMetricLine, nowMs));
      } catch (err) {
  ...
  private async executeForUser(runtime: LiquidationOiUserRuntimeRef, symbol: string, globalSignalId: string, side: Side, entryPrice: number, strategyInvalidationPrice: number, emergencyHardStopPrice: number, tpPrice: number, percentileRank: number, sameDirectionLiqUsd: number, counterMoveAtr: number, capacityAtr: number | null, netRR: number | null, orderBook: OrderBookObservation | null, oiMetricLine: string | null, nowMs: number): Promise<UserFanOutOutcome> {
    ...
    if (mode === "REAL") {
      ...
      const estimatedEmergencyMaxLossUsd = Math.abs(entryPrice - emergencyHardStopPrice) * sizing.positionQty;
      const emergencyLossMultiple = estimatedEmergencyMaxLossUsd / runtime.riskUsd;
      if (emergencyLossMultiple > this.strategyConfig.maxEmergencyLossMultipleOfRiskUsd) {
        userExec = { ...userExec, state: "TERMINAL", terminalReason: "EXECUTION_FAILED", cleanupState: "COMPLETE", updatedAt: nowMs };
        await this.globalSignalRepo.upsertUserExecution(userExec);
        log.warn(`[LOX_EMERGENCY_RISK_CONSTRAINT_VIOLATED] userId=${runtime.userId} symbol=${symbol} estimatedEmergencyMaxLossUsd=${estimatedEmergencyMaxLossUsd.toFixed(2)} riskUsd=${runtime.riskUsd} multiple=${emergencyLossMultiple.toFixed(2)}x exceeds maxEmergencyLossMultipleOfRiskUsd=${this.strategyConfig.maxEmergencyLossMultipleOfRiskUsd}x -- skipping rather than silently accepting larger risk`);
        return "FAILED";
      }
      userExec = { ...userExec, quantity: sizing.positionQty, positionSizeUsdt: sizing.positionSizeUsdt, estimatedStrategyLossUsd: runtime.riskUsd, estimatedEmergencyMaxLossUsd };
      await this.globalSignalRepo.upsertUserExecution(userExec);
      ...
      const outcome = await runEntrySequence(runtime.binanceRest, {
        ...
        entryPriceEstimate: entryPrice, emergencyStopPrice: emergencyHardStopPrice, initialTpPrice: tpPrice,
      });
      return await this.persistOutcome(userExec, outcome, symbol, side, strategyInvalidationPrice, emergencyHardStopPrice, tpPrice, percentileRank, sameDirectionLiqUsd, counterMoveAtr, capacityAtr, netRR, orderBook, oiMetricLine, runtime);
    }
    ...
    if (runtime.telegram !== null) {
      try {
        const text = formatEntryMessage({
          ...
          emergencyHardStopPrice: null, sameDirectionLiqUsd, percentileRank, oiMetricLine, counterMoveAtr, capacityAtr, netRR, orderBook,
  ...
  private async persistOutcome(userExec: LiquidationOiUserExecutionState, outcome: Awaited<ReturnType<typeof runEntrySequence>>, symbol: string, side: Side, strategyInvalidationPrice: number, emergencyHardStopPrice: number, tpPrice: number, percentileRank: number, sameDirectionLiqUsd: number, counterMoveAtr: number, capacityAtr: number | null, netRR: number | null, orderBook: OrderBookObservation | null, oiMetricLine: string | null, runtime: LiquidationOiUserRuntimeRef): Promise<UserFanOutOutcome> {
    const now = Date.now();
    ...
    await this.strategyOrderRepo.upsert({ userId: userExec.userId, globalSignalId: userExec.globalSignalId, symbol, purpose: "ENTRY", revision: 0, clientOrderId: outcome.entryClientOrderId, clientAlgoId: null, binanceOrderId: null, binanceAlgoId: null, state: "FILLED" });
    await this.strategyOrderRepo.upsert({ userId: userExec.userId, globalSignalId: userExec.globalSignalId, symbol, purpose: "EMERGENCY_STOP", revision: 0, clientOrderId: "", clientAlgoId: outcome.emergencyStopClientAlgoId, binanceOrderId: null, binanceAlgoId: outcome.emergencyStopBinanceAlgoId, state: "OPEN" });

    let updated: LiquidationOiUserExecutionState = {
      ...userExec, state: "ACTIVE", entryPrice: outcome.entryPrice, quantity: outcome.quantity,
      entryClientOrderId: outcome.entryClientOrderId, emergencyStopClientAlgoId: outcome.emergencyStopClientAlgoId,
      emergencyStopBinanceAlgoId: outcome.emergencyStopBinanceAlgoId, emergencyStopPrice: emergencyHardStopPrice,
      estimatedEmergencyMaxLossUsd: Math.abs(outcome.entryPrice - emergencyHardStopPrice) * outcome.quantity,
    ...
    if (runtime.telegram !== null) {
      try {
        const text = formatEntryMessage({
          ...
          emergencyHardStopPrice, sameDirectionLiqUsd, percentileRank, oiMetricLine, counterMoveAtr, capacityAtr, netRR, orderBook,
