...
export interface Trade {
   ...
   *  observation -- Binance's own aggregate trade id (`a` field).
   *  Optional (undefined for any existing Trade construction site that
   *  predates this and does not set it) -- used ONLY for de-duplicating
   *  after a WS reconnect/resubscribe, never for anything trading-
   *  relevant. */
  aggTradeId?: number;
}
