...
export function buildRealCloseReport(input: {
  ...
  const exitPrice = closing.reduce((t, f) => t + Number(f.price) * Number(f.qty), 0) / closedQty;
  const feesUsd = own.filter((f) => f.commissionAsset === "USDT").reduce((t, f) => t + Number(f.commission), 0);
  const grossPnl = closing.reduce((t, f) => t + Number(f.realizedPnl), 0);
  const ids = new Set(closing.map((f) => Number(f.orderId)));
  const reason = input.tpOrderId !== null && ids.has(input.tpOrderId)
    ? "TP_FILLED"
    : input.slActualOrderId !== null && ids.has(input.slActualOrderId)
      ? "SL_FILLED"
      : "POSITION_CLOSED_EXTERNALLY";
  return { reason, exitPrice, closedQty, realizedPnlUsd: grossPnl - feesUsd, feesUsd };
}
