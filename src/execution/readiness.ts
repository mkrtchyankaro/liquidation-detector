...
const log = childLogger({ mod: "readiness" });
 ...
 */
export async function checkRealReadiness(userId: string, rest: ReadinessRest): Promise<ReadinessResult> {
  let availableUsdt = 0;
  try {
  ...
  log.info(`[REAL_READY] userId=${userId} availableUsdt=${availableUsdt.toFixed(2)}`);
