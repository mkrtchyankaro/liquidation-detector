...
import { ensureMinuteBarIndexes, MinuteBarWriter } from "./collector/minute-bars";
...
import { mongoBarLoader, ZzPaperService, type ZzUserRef } from "./strategy/zz/zz-paper.service";
...
async function main(): Promise<void> {
  ...
  const zzUsers: ZzUserRef[] = config.zz.users.map((id) => users.find((u) => u.userId === id)).filter((u): u is UserConfig => !!u)
    .map((u) => ({ userId: u.userId, riskUsd: u.riskUsd, telegram: telegramOf(u) }));
  const zz = config.zz.enabled ? new ZzPaperService(config.zz, () => zzUsers, mongoBarLoader(mongo.db), mongo.db) : null;
  if (zz) void zz.start().catch((err) => log.error(`[ZZ_START_FAILED] ${err instanceof Error ? err.message : String(err)} -- ZZ PAPER not running, V9 unaffected`));
