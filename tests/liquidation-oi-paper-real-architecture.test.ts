...
import { pctMoveFromEntry, computePaperPnl } from "../src/domain/liquidation-oi-strategy/pnl-calculator";
...
function mockRestThrowsAlways(): BinanceRestLike {
  const fail = () => { throw new Error("THIS REST CLIENT MUST NEVER BE CALLED -- PAPER mode must place zero Binance calls, regardless of whether a client is configured"); };
...
async function main(): Promise<void> {
  ...
  await scenario("Matrix.1. enabled=false -> resolveUserExecutionMode returns NONE", () => {
  ...
  await scenario("Matrix.2. enabled=true, userExec=false -> PAPER regardless of global", () => {
  ...
  await scenario("Matrix.3. enabled=true, userExec=true, global=false -> PAPER (safety fallback)", () => {
    assert.strictEqual(resolveUserExecutionMode(true, true, false), "PAPER");
  });
  await scenario("Matrix.4. enabled=true, userExec=true, global=true -> REAL", () => {
    assert.strictEqual(resolveUserExecutionMode(true, true, true), "REAL");
  });

  await scenario("PnL.1. LONG TP%/SL% signs correct", () => {
    assert.ok(pctMoveFromEntry("LONG", 100, 101) > 0, "LONG favorable move must be positive");
    assert.ok(pctMoveFromEntry("LONG", 100, 99) < 0, "LONG adverse move must be negative");
  });
  await scenario("PnL.2. SHORT TP%/SL% signs correct", () => {
    assert.ok(pctMoveFromEntry("SHORT", 100, 99) > 0, "SHORT favorable move (price down) must be positive");
    assert.ok(pctMoveFromEntry("SHORT", 100, 101) < 0, "SHORT adverse move (price up) must be negative");
  });
  await scenario("PnL.3. paper gross PnL matches direction for LONG and SHORT", () => {
    const long = computePaperPnl({ side: "LONG", entryPrice: 100, exitPrice: 105, quantity: 2 });
    assert.strictEqual(long.grossPnlUsd, 10);
    const short = computePaperPnl({ side: "SHORT", entryPrice: 100, exitPrice: 95, quantity: 2 });
    assert.strictEqual(short.grossPnlUsd, 10);
  });

  await scenario("P.1. global OFF -> both users PAPER, paper entry persists with entry/TP/SL, zero Binance calls even though a real client is configured", async () => {
    const karoRest = mockRestThrowsAlways();
    const artakRest = mockRestThrowsAlways();
    const { orch, userExecs, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest: karoRest }, { userId: "artak", riskUsd: 5, rest: artakRest }]), false);
    ...
    assert.strictEqual(karo.mode, "PAPER"); assert.strictEqual(artak.mode, "PAPER");
    assert.strictEqual(karo.state, "ACTIVE");
    assert.ok(karo.entryPrice !== null && karo.tpPrice !== null && karo.quantity !== null);
    assert.strictEqual(signals.docs.find((d: any) => d.state === "ACTIVE")?.state, "ACTIVE", "global signal remains ACTIVE after ENTRY, not just an observational record");
  });

  await scenario("P.2. PAPER TP hit -> terminal TP_FILLED with correct gross PnL, cleanup COMPLETE immediately, global closes once all paper users terminal", async () => {
    const rest = mockRestThrowsAlways();
    const { orch, activeMain, userExecs, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest }]), false);
    await driveToActive(orch, "SOLUSDT", 2_000_000);
    const karo = userExecs.docs.find((d: any) => d.userId === "karo");
    const signal = signals.docs.find((d: any) => d.state === "ACTIVE");
    await activeMain.onActiveTick("SOLUSDT", signal.globalSignalId, "ep-test", "SHORT", karo.tpPrice, 4590, 1.0, 2_010_000);
    ...
    assert.strictEqual(updated.cleanupState, "COMPLETE", "paper cleanup is immediate, no Binance scan needed");
    const expectedPnl = computePaperPnl({ side: "SHORT", entryPrice: karo.entryPrice, exitPrice: karo.tpPrice, quantity: karo.quantity });
    assert.ok(Math.abs(updated.grossPnlUsd - expectedPnl.grossPnlUsd) < 1e-9);
    assert.ok(signals.docs.some((d: any) => d.state === "CLOSED"), "global must CLOSE once the only manageable user is terminal+clean");
  });

  await scenario("P.3. PAPER strategy invalidation -> virtual close at causal reference price, correct PnL, zero Binance calls", async () => {
    const rest = mockRestThrowsAlways();
    const { orch, activeMain, userExecs, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest }]), false);
    await driveToActive(orch, "SOLUSDT", 3_000_000);
    const signal = signals.docs.find((d: any) => d.state === "ACTIVE");
    await activeMain.onActiveTick("SOLUSDT", signal.globalSignalId, "ep-test", "SHORT", signal.strategyInvalidationPrice + 0.5, 4590, 1.0, 3_010_000);
    const karo = userExecs.docs.find((d: any) => d.userId === "karo");
    assert.strictEqual(karo.terminalReason, "STRATEGY_INVALIDATION");
    assert.strictEqual(karo.exitPrice, signal.strategyInvalidationPrice + 0.5);
    assert.strictEqual(karo.cleanupState, "COMPLETE");
  });

  await scenario("P.4. PAPER follows MAIN dynamic TP revision -- currentTpPrice updates in Mongo, zero Binance calls", async () => {
    const rest = mockRestThrowsAlways();
    const { orch, activeMain, userExecs, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest }]), false);
    ...
    // the second call, after oiEfficiencyEvalIntervalMs, supplies MEANINGFULLY stronger post-entry
    // evidence (both genuine OI creation and favorable price move) than the initial stubbed capacity
    // (0.5 ATR) used at entry, so the real (unstubbed) dynamic capacity model produces a materially
    // larger remaining capacity and a genuine EXTEND_TP -- values verified via the real formula.
    await activeMain.onActiveTick("SOLUSDT", signal.globalSignalId, "ep-test", "SHORT", 102.5, 4590, 1.0, 4_010_000);
    await activeMain.onActiveTick("SOLUSDT", signal.globalSignalId, "ep-test", "SHORT", 101.6, 4590 * 5, 1.0, 4_010_000 + DEFAULT_ACTIVE_LIFECYCLE_CONFIG.oiEfficiencyEvalIntervalMs + 100);
    const karoAfter = userExecs.docs.find((d: any) => d.userId === "karo");
    assert.notStrictEqual(karoAfter.tpPrice, oldTp, "paper TP must follow MAIN's dynamic revision");
    assert.ok(karoAfter.appliedTpRevision > 0);
  });

  await scenario("P.5. MIXED: Karo REAL, Artak PAPER -- Artak's paper TP hits first, Karo stays REAL ACTIVE, global stays open; Karo later strategy-invalidates and closes -> GLOBAL CLOSED", async () => {
    const karoRest = mockRestSuccess();
    const artakRest = mockRestThrowsAlways();
    const { orch, activeMain, positionLifecycle, userExecs, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest: karoRest, enabled: true }, { userId: "artak", riskUsd: 5, rest: artakRest, enabled: false }]), true);
    ...
    await activeMain.onActiveTick("SOLUSDT", signal.globalSignalId, "ep-test", "SHORT", artak.tpPrice, 4590, 1.0, 5_010_000);
    ...
    assert.strictEqual(karo.state, "ACTIVE", "Karo's REAL position must be completely unaffected by Artak's paper TP hit");
    assert.ok(!signals.docs.some((d: any) => d.state === "CLOSED"), "global must not close while Karo remains REAL ACTIVE");

    karoRest.calls.length = 0;
    await activeMain.onActiveTick("SOLUSDT", signal.globalSignalId, "ep-test", "SHORT", signal.strategyInvalidationPrice + 1, 4590, 1.0, 5_020_000);
    assert.ok(karoRest.calls.some((c) => c.startsWith("createOrder:MARKET")), "Karo (REAL) must receive an actual reduce-only MARKET close");
    (karoRest as any).getPositionRisk = async () => [{ symbol: "SOLUSDT", positionAmt: "0", entryPrice: "98" }];
    (karoRest as any).getOrder = async () => ({ status: "FILLED" });
    await positionLifecycle.reconcileAll(5_030_000);
    karo = userExecs.docs.find((d: any) => d.userId === "karo");
    assert.strictEqual(karo.cleanupState, "COMPLETE");
    assert.ok(signals.docs.some((d: any) => d.state === "CLOSED"), "global closes once BOTH paper and real users are terminal+clean");
  });

  await scenario("P.6. TP revision NEVER places a real Binance TP for a PAPER user, even when that user happens to have a real client configured", async () => {
    const rest = mockRestThrowsAlways();
    const { orch, activeMain, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest, enabled: true }]), false);
    await driveToActive(orch, "SOLUSDT", 6_000_000);
    const signal = signals.docs.find((d: any) => d.state === "ACTIVE");
    await activeMain.onActiveTick("SOLUSDT", signal.globalSignalId, "ep-test", "SHORT", 95, 4590 * 1.05, 1.0, 6_010_000);
  });

  await scenario("P.7. MARKET_EXIT never places a real Binance order for a PAPER user, even when that user happens to have a real client configured", async () => {
    const rest = mockRestThrowsAlways();
    const { orch, activeMain, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest, enabled: true }]), false);
    await driveToActive(orch, "SOLUSDT", 7_000_000);
    const signal = signals.docs.find((d: any) => d.state === "ACTIVE");
    await activeMain.onActiveTick("SOLUSDT", signal.globalSignalId, "ep-test", "SHORT", signal.strategyInvalidationPrice + 1, 4590, 1.0, 7_010_000);
  });

  await scenario("P.8. REGRESSION (live production bug, Sep 17 2026) -- the periodic Binance reconciler must NEVER treat a PAPER user's real (always-flat) Binance position as POSITION_CLOSED_EXTERNALLY, even when that user has a real, working binanceRest client configured", async () => {
    const rest = mockRestSuccess();
    (rest as any).getPositionRisk = async () => [{ symbol: "SOLUSDT", positionAmt: "0", entryPrice: "0" }]; // exactly the real-world case: a real client, but flat, because no real trade was ever placed for this PAPER user
    const { orch, positionLifecycle, userExecs, signals } = buildStack(runtimes([{ userId: "brother", riskUsd: 1, rest, enabled: false }]), true);
    ...
    assert.strictEqual(after.state, "ACTIVE", "a PAPER user must remain ACTIVE through Binance reconciliation -- it is monitored exclusively by the causal price-based paper checks, never by real-position polling");
