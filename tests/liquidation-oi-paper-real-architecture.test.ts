...
async function driveToActive(
  ...
  const historyAtEpisodeEnd = [
    ...
    { contracts: 4750, fetchedAt: now0 + 180_000 },
  ];
  await orch.onTick(
    symbol,
    PCTX,
  ...
  const historyWithCreation = [
    ...historyAtEpisodeEnd,
    { contracts: 4900, fetchedAt: now0 + 200_000 },
  ];
  await orch.onTick(
    symbol,
    PCTX,
    ...
    {
      capacityAtr: 0.3,
      candidateTpPrice: 101.4,
      candidateSlPrice: 103.1,
      netRR: 2.5,
    },
  );
}
...
async function main(): Promise<void> {
  ...
  await scenario(
    ...
    async () => {
      ...
      // Sep 19 2026 (Karo), operator-requested REVISION -- STRATEGY_INVALIDATION
      // is now a no-op for REAL users via MAIN's own onActiveTick (Binance's
      // resting SL order, placed at entry, is the sole mechanism now). Simulate
      // that resting order filling on Binance's own side instead: position
      // goes flat, the SL algo order reports FILLED, and reconcileAll() is
      // what actually detects and reports the close -- not onActiveTick.
      await activeMain.onActiveTick(
        "SOLUSDT",
        signal.globalSignalId,
        "ep-test",
      ...
      assert.ok(
        ...
        "Karo (REAL) must NOT receive a MARKET close from MAIN for STRATEGY_INVALIDATION anymore -- Binance's own resting SL order is now the sole mechanism",
      );
      karo = userExecs.docs.find((d: any) => d.userId === "karo");
      assert.strictEqual(
        karo.state,
        "ACTIVE",
        "MAIN's in-process trigger must leave REAL state untouched for this reason",
      );
      (karoRest as any).getPositionRisk = async () => [
        { symbol: "SOLUSDT", positionAmt: "0", entryPrice: "98" },
      ];
      (karoRest as any).getAlgoOrder = async () => ({ algoStatus: "FILLED" });
      await positionLifecycle.reconcileAll(5_030_000);
      karo = userExecs.docs.find((d: any) => d.userId === "karo");
      assert.strictEqual(
        karo.terminalReason,
        "SL_FILLED",
        "reconciliation must correctly attribute the close to the resting SL order's own fill status",
      );
      assert.strictEqual(karo.cleanupState, "COMPLETE");
      assert.ok(
        signals.docs.some((d: any) => d.state === "CLOSED"),
