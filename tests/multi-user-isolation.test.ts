...
async function main(): Promise<void> {
  ...
  await scenario("ONE V5 strategy signal has exactly ONE canonical signalId", () => {
    const g1 = makeGlobalSignal("SIG-ABC123");
    const g2 = makeGlobalSignal("SIG-ABC123");
    assert.strictEqual(g1.signalId, g2.signalId);
  });

  await scenario("the SAME signalId is independently persisted into v5_signals_karo AND v5_signals_friend (two SEPARATE collections)", async () => {
  ...
  await scenario("a THIRD user (artak) who was never given this signalId has NO document for it at all", async () => {
    const store = new FakePerUserSignalStore();
    await store.upsert("karo", makeUserSignal("SIG-XYZ"));
    assert.strictEqual(store.hasAny("artak", "SIG-XYZ"), false);
  });

  await scenario("Karo's manual close updates ONLY v5_signals_karo -- v5_signals_friend for the SAME signalId remains untouched", async () => {
    const store = new FakePerUserSignalStore();
    const signalId = "SIG-ABC123";
    await store.upsert("karo", makeUserSignal(signalId));
    await store.upsert("friend", makeUserSignal(signalId));
    const karoDoc = await store.findBySignalId("karo", signalId);
    assert.ok(karoDoc);
    await store.upsert("karo", { ...karoDoc, status: "CLOSED_MANUAL", isLive: false, closedAt: Date.now(), closeReason: "MANUAL" });
  ...
  await scenario("the GLOBAL canonical signal document itself is never mutated by any user's own close -- structurally cannot hold a per-user close status", () => {
    const g = makeGlobalSignal("SIG-ABC123");
    assert.strictEqual(g.status, "SIGNAL");
  });

  await scenario("one user's own daily-loss-limit block does not affect another user's own tracker", () => {
    const now = Date.now();
  ...
  await scenario("preserved calculation/fallback behavior: unconfigured tracker falls back to the EXACT original 500/5 defaults", () => {
    const now = Date.now();
  ...
  await scenario("per-user execution claims are scoped per user (structural proof: constructor requires userId, collection name includes it)", () => {
    const src = fs.readFileSync(path.join(__dirname, "../src/infrastructure/mongo/execution-claim.repository.ts"), "utf8");
    assert.ok(src.includes("userId: string"));
    const mongoClientSrc = fs.readFileSync(path.join(__dirname, "../src/infrastructure/mongo/mongo.client.ts"), "utf8");
