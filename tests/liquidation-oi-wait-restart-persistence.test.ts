...
class FakeCollection<T extends Record<string, unknown>> {
  ...
  async updateOne(filter: Record<string, unknown>, update: { $set?: Partial<T> }, opts?: { upsert?: boolean }): Promise<{ matchedCount: number }> {
    ...
    if (opts?.upsert) this.docs.push({ ...(filter as Partial<T>), ...(update.$set ?? {}) } as T);
    return { matchedCount: 0 };
  }
  async deleteOne(filter: Record<string, unknown>): Promise<{ deletedCount: number }> {
    const before = this.docs.length;
    this.docs = this.docs.filter((d) => !Object.entries(filter).every(([k, v]) => (d as Record<string, unknown>)[k] === v));
    return { deletedCount: before - this.docs.length };
  }
  find(): { toArray: () => Promise<T[]> } { return { toArray: async () => [...this.docs] }; }
}
...
function noRuntimes(): LiquidationOiUserRuntimeRef[] { return []; }
...
function buildOrch(mongo: MongoClientWrapper) {
  ...
  const orch = new LiquidationOiRuntimeOrchestrator(
    TEST_STRATEGY_CONFIG, DEFAULT_CAPACITY_MODEL_COEFFICIENTS, globalSignalRepo, strategyOrderRepo, noRuntimes,
    true, false, undefined, () => {}, undefined, undefined, waitStateRepo,
  );
  return { orch, waitStateRepo };
}

async function driveToWait(orch: LiquidationOiRuntimeOrchestrator, symbol: string, now0: number): Promise<void> {
...
async function main(): Promise<void> {
  ...
  await scenario("1. entering WAIT_FOR_POST_EPISODE_OI_CREATION persists a doc", async () => {
    const { mongo, waitStates } = fakeMongoWithWaitStates();
    const { orch } = buildOrch(mongo);
    await driveToWait(orch, "SOLUSDT", 4_000_000);
    assert.strictEqual(orch.getWatchManager().getLifecycle("SOLUSDT")?.globalState, "WAIT_FOR_POST_EPISODE_OI_CREATION");
    assert.strictEqual(waitStates.docs.length, 1, "a WAIT doc must be persisted");
    assert.strictEqual(waitStates.docs[0]!.symbol, "SOLUSDT");
  });

  await scenario("2. restart restores the symbol into WAIT_FOR_POST_EPISODE_OI_CREATION in a FRESH orchestrator instance, with ownership, without duplicating the episode", async () => {
    const { mongo, waitStates } = fakeMongoWithWaitStates();
    ...
    assert.strictEqual(before.globalState, "WAIT_FOR_POST_EPISODE_OI_CREATION");
    assert.strictEqual(waitStates.docs.length, 1);

    const { orch: orch2 } = buildOrch(mongo);
    assert.strictEqual(orch2.getWatchManager().getLifecycle("AVAXUSDT"), null, "fresh instance starts with nothing in memory");
    ...
    assert.strictEqual(after.globalState, "WAIT_FOR_POST_EPISODE_OI_CREATION", "must restore directly into WAIT, not re-derive it");
    assert.strictEqual(after.episode.sameDirectionLiqUsd, before.episode.sameDirectionLiqUsd);
    assert.strictEqual(after.episode.extremePrice, before.episode.extremePrice);
    assert.strictEqual(after.episodeEndOiQuantity, before.episodeEndOiQuantity);
    assert.strictEqual(after.episodeEndPrice, before.episodeEndPrice);
    assert.strictEqual(after.ownershipId, before.ownershipId, "ownershipId must be preserved so symbol ownership is not lost");

    const episodeIdBefore = after.episodeId;
    orch2.onLiquidationEvent({ symbol: "AVAXUSDT", victim: "SHORT", timestamp: 4_400_100, price: 101.9, quoteQty: 50000 }, null);
    const afterEvent = orch2.getWatchManager().getLifecycle("AVAXUSDT")!;
    assert.strictEqual(afterEvent.episodeId, episodeIdBefore, "must fold into the SAME restored episode, never start a competing one");
    assert.ok(afterEvent.episode.sameDirectionLiqUsd > before.episode.sameDirectionLiqUsd);
  });

  await scenario("3. restart does not enter merely because it occurred -- the same causal economic gate still governs entry afterward", async () => {
    ...
    assert.strictEqual(orch2.getWatchManager().getLifecycle("ETHUSDT")?.globalState, "WAIT_FOR_POST_EPISODE_OI_CREATION");

    const history = [{ contracts: 4590, fetchedAt: 4_400_000 }];
    await orch2.onTick("ETHUSDT", PCTX, history, 101.8, 1.0, 1000, 4_400_100, null, null, null, [], [], FLAT_ATR);
    assert.strictEqual(orch2.getWatchManager().getLifecycle("ETHUSDT")?.globalState, "WAIT_FOR_POST_EPISODE_OI_CREATION", "must remain WAIT -- restart itself proves nothing about market evidence");
  });
