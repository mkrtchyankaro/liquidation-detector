...
async function main(): Promise<void> {
  console.log("Running operational-safety (stuck-signal / reset-tool) regression tests...\n");

  await scenario("4. all users already TERMINAL+COMPLETE before restart -> global self-heals to CLOSED on restart", async () => {
    const { orch, globalSignalRepo, strategyOrderRepo, positionLifecycle, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest: mockRestThrowsAlways() }]), false);
    await driveToActive(orch, "ADAUSDT", 1_000_000);
    const signal = signals.docs.find((d: any) => d.state === "ACTIVE");
    const userExecRow = (await globalSignalRepo.findUserExecutionsForSignal(signal.globalSignalId))[0];
    await globalSignalRepo.upsertUserExecution({ ...userExecRow, state: "TERMINAL", terminalReason: "TP_FILLED", cleanupState: "COMPLETE" });
    assert.strictEqual(signals.docs.find((d: any) => d.globalSignalId === signal.globalSignalId).state, "ACTIVE", "sanity: global is stuck ACTIVE before restart");

    await recoverLoxOnRestart(globalSignalRepo, strategyOrderRepo, positionLifecycle, orch.getWatchManager(), runtimes([{ userId: "karo", riskUsd: 1, rest: mockRestThrowsAlways() }]), () => {}, 1_100_000);

    const after = signals.docs.find((d: any) => d.globalSignalId === signal.globalSignalId);
    assert.strictEqual(after.state, "CLOSED", "restart must self-heal a signal whose every user was already terminal+clean before the crash");
  });

  await scenario("5. a genuinely valid ACTIVE signal survives restart UNCHANGED and stays locked", async () => {
    const { orch, globalSignalRepo, strategyOrderRepo, positionLifecycle, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest: mockRestThrowsAlways() }]), false);
    await driveToActive(orch, "ADAUSDT", 2_000_000);
    const signal = signals.docs.find((d: any) => d.state === "ACTIVE");

    await recoverLoxOnRestart(globalSignalRepo, strategyOrderRepo, positionLifecycle, orch.getWatchManager(), runtimes([{ userId: "karo", riskUsd: 1, rest: mockRestThrowsAlways() }]), () => {}, 2_100_000);

    const after = signals.docs.find((d: any) => d.globalSignalId === signal.globalSignalId);
    assert.strictEqual(after.state, "ACTIVE", "a genuinely still-managed PAPER position must not be closed merely by restarting");
  });

  await scenario("restart restores the in-memory symbol lock for a genuinely still-ACTIVE signal -- a fresh opposite-direction liquidation must NOT steal the symbol", async () => {
    const { orch, globalSignalRepo, strategyOrderRepo } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest: mockRestThrowsAlways() }]), false);
    await driveToActive(orch, "ADAUSDT", 3_000_000);
    const { orch: freshOrch, positionLifecycle: freshPositionLifecycle } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest: mockRestThrowsAlways() }]), false);
    assert.strictEqual(freshOrch.getWatchManager().getLifecycle("ADAUSDT"), null, "sanity: a brand new process starts with an empty in-memory map");

    await recoverLoxOnRestart(globalSignalRepo, strategyOrderRepo, freshPositionLifecycle, freshOrch.getWatchManager(), runtimes([{ userId: "karo", riskUsd: 1, rest: mockRestThrowsAlways() }]), () => {}, 3_100_000);

    assert.strictEqual(freshOrch.getWatchManager().getLifecycle("ADAUSDT")?.globalState, "ACTIVE", "restart must restore the in-memory lock for a genuinely still-ACTIVE signal");
    assert.strictEqual(freshOrch.getWatchManager().isSymbolOwned("ADAUSDT"), true);
    freshOrch.onLiquidationEvent({ symbol: "ADAUSDT", victim: "LONG", timestamp: 3_200_000, price: 90, quoteQty: 60000 }, null);
    assert.strictEqual(freshOrch.getWatchManager().getLifecycle("ADAUSDT")?.globalState, "ACTIVE", "the restored ACTIVE lifecycle must be completely unaffected by a post-restart opposite event");
  });

  await scenario("6. reset dry-run (audit only) performs ZERO writes", async () => {
    const { signals, userExecs, orders } = fakeMongo();
    signals.docs.push({ globalSignalId: "sig-1", symbol: "ADAUSDT", state: "ACTIVE", victim: "SHORT" } as any);
    userExecs.docs.push({ userId: "karo", globalSignalId: "sig-1", symbol: "ADAUSDT", mode: "PAPER", state: "ACTIVE", cleanupState: "PENDING" } as any);
    const cols: LoxCollections = { signals: signals as any, userExecs: userExecs as any, orders: orders as any };
    await auditLoxState(cols);
    assert.strictEqual(signals.docs.length, 1, "dry-run audit must not remove anything");
    assert.strictEqual(userExecs.docs.length, 1);
  });

  await scenario("7. reset detection correctly flags REAL exposure and would refuse", async () => {
    const { signals, userExecs, orders } = fakeMongo();
    userExecs.docs.push({ userId: "karo", globalSignalId: "sig-real", symbol: "ADAUSDT", mode: "REAL", state: "ACTIVE", cleanupState: "PENDING" } as any);
    userExecs.docs.push({ userId: "artak", globalSignalId: "sig-real2", symbol: "SOLUSDT", mode: "REAL", state: "TERMINAL", cleanupState: "FAILED_RETRYING", cleanupFailureReason: "api down" } as any);
    orders.docs.push({ userId: "artak", globalSignalId: "sig-real2", symbol: "SOLUSDT", purpose: "TAKE_PROFIT", state: "OPEN" } as any);
    const cols: LoxCollections = { signals: signals as any, userExecs: userExecs as any, orders: orders as any };
    const audit = await auditLoxState(cols);
    assert.ok(audit.realExposure.length >= 2, "must detect real exposure from BOTH the ACTIVE+PENDING row and the FAILED_RETRYING+unresolved-order row");
    assert.ok(audit.realExposure.some((r) => r.userId === "karo"));
    assert.ok(audit.realExposure.some((r) => r.userId === "artak"));
  });

  await scenario("10. shared strategy_orders collection: only rows with a genuine globalSignalId are treated as LOX-owned", async () => {
    const { signals, userExecs, orders } = fakeMongo();
    orders.docs.push({ userId: "karo", globalSignalId: "sig-1", symbol: "ADAUSDT", purpose: "ENTRY", state: "FILLED" } as any);
    orders.docs.push({ userId: "someone", globalSignalId: "", symbol: "ADAUSDT", purpose: "OTHER", state: "OPEN" } as any);
    const cols: LoxCollections = { signals: signals as any, userExecs: userExecs as any, orders: orders as any };
    const audit = await auditLoxState(cols);
    assert.strictEqual(audit.allOrders.length, 1, "only the row with a non-empty globalSignalId must be counted as LOX-owned");
  });

  await scenario("11. confirmed reset (simulated) leaves zero LOX active locks", async () => {
    const { signals, userExecs, orders } = fakeMongo();
    signals.docs.push({ globalSignalId: "sig-1", symbol: "ADAUSDT", state: "CLOSED" } as any);
    userExecs.docs.push({ userId: "karo", globalSignalId: "sig-1", symbol: "ADAUSDT", mode: "PAPER", state: "TERMINAL", cleanupState: "COMPLETE" } as any);
    orders.docs.push({ userId: "karo", globalSignalId: "sig-1", symbol: "ADAUSDT", purpose: "ENTRY", state: "FILLED" } as any);
