...
async function main(): Promise<void> {
  ...
  await scenario(
    "MP.1. tiny same-side liquidation noise cannot keep a dead episode alive forever",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        noProgressTimeoutMs: 60_000,
      ...
      assert.ok(
        mgr.getLifecycle("BTCUSDT") !== null,
        "sanity: still alive after 200s of tiny noise (under noProgressTimeoutMs=60s since the LAST tick, but no single event was individually meaningful)",
      );
      ...
      assert.strictEqual(
        mgr.getLifecycle("BTCUSDT"),
        null,
        "must die once meaningful progress (not mere activity) has been absent for noProgressTimeoutMs",
      );
    },
  );

  await scenario(
    "MP.2. tiny adverse price extensions cannot keep a dead episode alive forever",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        noProgressTimeoutMs: 60_000,
      ...
      assert.strictEqual(
        mgr.getLifecycle("BTCUSDT"),
        null,
        "noisy sub-threshold extreme extension must not indefinitely refresh the clock",
      );
    },
  );

  await scenario(
    "MP.3. meaningful continuing liquidation pressure DOES keep an episode alive",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        noProgressTimeoutMs: 60_000,
      ...
      assert.ok(
        mgr.getLifecycle("BTCUSDT") !== null,
        "genuinely continuing meaningful liquidation pressure must keep the episode alive",
      );
    },
  );

  await scenario(
    "MP.4. meaningful adverse displacement DOES keep an episode alive",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        noProgressTimeoutMs: 60_000,
      ...
      assert.ok(
        mgr.getLifecycle("BTCUSDT") !== null,
        "sanity: alive right after the meaningful extension",
      );
      mgr.onTick("BTCUSDT", PCTX, [], 75710, 100, 1000, 50_001 + 59_000);
      assert.ok(
        mgr.getLifecycle("BTCUSDT") !== null,
        "meaningful adverse displacement must extend the episode's life by refreshing the checkpoint",
      );
    },
  );

  await scenario(
    "MP.5. continuing OI destruction progress can keep a genuinely active episode alive",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        noProgressTimeoutMs: 60_000,
      ...
      mgr.onLiquidationEvent(
        ...
        { quantity: 95000, timestamp: 50_000 },
      ); // -5% of starting OI
      ...
      assert.ok(
        mgr.getLifecycle("BTCUSDT") !== null,
        "continuing meaningful OI destruction alone must be able to keep a genuinely active episode alive",
      );
    },
  );

  await scenario(
    "MP.6. once flow/progress dies, a later same-side liquidation starts a NEW episode, not an accumulation into the ancient one",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        noProgressTimeoutMs: 60_000,
      ...
      assert.strictEqual(
        mgr.getLifecycle("BTCUSDT"),
        null,
        "the ancient episode must be dead",
      );
      mgr.onLiquidationEvent(
        {
          symbol: "BTCUSDT",
      ...
      assert.strictEqual(
        lc.episode.eventCount,
        1,
        "must be a fresh episode -- eventCount=1, not accumulated onto the dead one",
      );
  ...
  await scenario(
    "MP.7. once released, an opposite-side liquidation can immediately start a new episode",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        noProgressTimeoutMs: 60_000,
      ...
      assert.strictEqual(
        lc.episode.victim,
        "LONG",
        "reproduces the real $2.53M cascade -- must be evaluated as its own fresh episode, immediately",
      );
  ...
  await scenario(
    "MP.8. transient lack of progress does not prematurely kill a valid active burst",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        noProgressTimeoutMs: 60_000,
  ...
  await scenario(
    "MP.9. hard failsafe still works as a final safety valve even with meaningful-progress tracking active",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        preEntryFailsafeMaxLifetimeMs: 100_000,
      ...
      assert.strictEqual(
        mgr.getLifecycle("BTCUSDT"),
        null,
        "FAILSAFE must fire regardless of meaningful progress once the absolute cap is exceeded",
      );
      assert.ok(
        mgr
          .getNoSignalLog()
  ...
  await scenario(
    "MP.10. ACTIVE real positions are completely unaffected by all pre-entry death logic, meaningful-progress fix included",
    async () => {
      const { mongo } = fakeMongo();
      const rest = mockRestSuccess();
      const orch = new LiquidationOiRuntimeOrchestrator(
  ...
  await scenario(
    "MP.11. no symbol can remain indefinitely owned in any non-ACTIVE theoretical state (direct proof across all three checkpoints)",
    () => {
      const config = {
        ...DEFAULT_LIQUIDATION_OI_STRATEGY_CONFIG,
        noProgressTimeoutMs: 60_000,
      ...
      for (let i = 0; i < 50; i++) {
        ...
        mgr.onLiquidationEvent(
          ...
          { quantity: 99999, timestamp: t },
        );
        mgr.onTick("BTCUSDT", PCTX, [], 75700, 100, 1000, t + 1);
        if (before !== null && mgr.getLifecycle("BTCUSDT") === null)
          deathCount++;
      }
      assert.ok(
        deathCount >= 20,
        `expected the pure-noise episode to die repeatedly across the ~${(t / 3_600_000).toFixed(1)}h window (got ${deathCount} deaths) -- proving no single noisy episode persisted indefinitely`,
      );
      ...
      assert.strictEqual(
        mgr.getLifecycle("BTCUSDT"),
        null,
        "the final live episode must also die once its own noProgressTimeoutMs elapses -- nothing here is exempt",
      );
    },
  );
