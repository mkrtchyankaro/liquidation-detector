...
async function driveLongEpisodeToActive(
  ...
  symbol: string,
  now0: number,
): Promise<{ tick: (price: number, tMs: number) => Promise<void> }> {
...
function buildFreshRuntimeOnSameMongo(
...
async function main(): Promise<void> {
  console.log(
    "Running the ETHUSDT stuck-signal regression tests (real orch.onTick() wiring)...\n",
  );

  await scenario(
    "REGRESSION (live production bug): onTick()'s ACTIVE branch must pass globalSignalId, not ownershipId, to onActiveTick",
    async () => {
      const rest = mockRestThrowsAlways();
      const { orch, signals, userExecs } = buildStack(
        runtimes([{ userId: "main", riskUsd: 10, rest }]),
        false,
      );
      const { tick } = await driveLongEpisodeToActive(
        orch,
        "ETHUSDT",
        1_000_000,
      );
      ...
      assert.strictEqual(
        after.state,
        "CLOSED",
        "with the fix, a real tick through orch.onTick() must reach onActiveTick and trigger strategy invalidation",
      );
  ...
  await scenario(
    "CASE A (exact ETH shape): LONG PAPER entry=2431.52 tp=2437.62 SL=2430.80 -- stays ACTIVE below TP; hits TP_FILLED at TP, exitPrice/PnL populated, CLOSE once, cleanup COMPLETE",
    async () => {
      const rest = mockRestThrowsAlways();
      const { orch, signals, userExecs } = buildStack(
        runtimes([{ userId: "main", riskUsd: 10, rest }]),
      ...
      assert.strictEqual(
        ...
        "ACTIVE",
        "must remain ACTIVE just below TP",
      );
      ...
      assert.strictEqual(
        user.exitPrice,
        tpPrice,
        "exitPrice must be populated",
      );
      assert.ok(
        user.grossPnlUsd !== null && user.grossPnlUsd > 0,
        "PnL must be populated and positive for a LONG TP hit",
      );
      assert.strictEqual(user.cleanupState, "COMPLETE");
      const closedSignal = signals.docs.find(
        (d: any) => d.globalSignalId === signal.globalSignalId,
      );
      ...
      const userAfterExtraTick = userExecs.docs.find(
        (d: any) => d.userId === "main",
      );
      assert.strictEqual(
        userAfterExtraTick.grossPnlUsd,
        pnlBefore,
        "terminal transition must be idempotent -- no duplicate CLOSE/PnL recompute",
      );
    },
  );

  await scenario(
    "CASE B (exact ETH shape), fresh signal: stays ACTIVE above SL; hits STRATEGY_INVALIDATION at SL, exitPrice/PnL populated, CLOSE once, cleanup COMPLETE",
    async () => {
      const rest = mockRestThrowsAlways();
      const { orch, signals, userExecs } = buildStack(
        runtimes([{ userId: "main", riskUsd: 10, rest }]),
      ...
      assert.strictEqual(
        ...
        "ACTIVE",
        "must remain ACTIVE just above SL",
      );
      ...
      assert.ok(
        user.grossPnlUsd !== null && user.grossPnlUsd < 0,
        "PnL must be populated and negative for a LONG SL hit",
      );
      assert.strictEqual(user.cleanupState, "COMPLETE");
      assert.strictEqual(
        signals.docs.find(
  ...
  await scenario(
    "SHORT symmetric: TP below entry, SL above entry, both trigger correctly through the real tick path",
    async () => {
      const rest = mockRestThrowsAlways();
      const { orch, signals, userExecs } = buildStack(
        runtimes([{ userId: "main", riskUsd: 10, rest }]),
      ...
      orch.onLiquidationEvent(
        ...
        { quantity: 6000, timestamp: 4_000_000 },
      );
      orch.onLiquidationEvent(
        {
          symbol: "ETHUSDT",
  ...
  await scenario(
    "Mixed PAPER + REAL: both users' active monitoring is reached through the real tick path",
    async () => {
      ...
      const realRest: BinanceRestLike = {
        ...
        createOrder: async () => ({ orderId: 1 }),
        createAlgoOrder: async () => ({ algoId: 1 }),
      ...
      const { orch, signals, userExecs } = buildStack(
        runtimes([
          { userId: "paperuser", riskUsd: 10, rest: paperRest, enabled: false },
          { userId: "realuser", riskUsd: 10, rest: realRest, enabled: true },
        ]),
        true,
      );
      const { tick } = await driveLongEpisodeToActive(
      ...
      const signal = signals.docs.find((d: any) => d.state === "ACTIVE");
      const paperBefore = userExecs.docs.find(
        (d: any) => d.userId === "paperuser",
      );
      const realBefore = userExecs.docs.find(
        (d: any) => d.userId === "realuser",
      );
      ...
      const paperAfter = userExecs.docs.find(
        (d: any) => d.userId === "paperuser",
      );
      assert.strictEqual(
        paperAfter.state,
        "TERMINAL",
        "PAPER user's active monitoring must be reached through the real tick path too",
      );
  ...
  await scenario(
    "RESTART: a persisted ACTIVE PAPER signal is reattached to live monitoring after restart -- a subsequent real tick above TP closes it",
    async () => {
      const rest = mockRestThrowsAlways();
      const { orch, signals, userExecs, globalSignalRepo, strategyOrderRepo } =
        buildStack(runtimes([{ userId: "main", riskUsd: 10, rest }]), false);
      ...
      const restoredSignal = signals.docs.find(
        (d: any) => d.state === "ACTIVE",
      );
      assert.ok(
        restoredSignal,
        "signal must still be ACTIVE after restart (never closed merely for being old)",
      );

      const historyPost = [{ contracts: 5480, fetchedAt: 6_050_000 }];
      await freshOrch.onTick(
        "ETHUSDT",
        PCTX,
        historyPost,
        restoredSignal.initialTpPrice,
        1.0,
        1000,
        6_200_000,
      );
      ...
      assert.strictEqual(
        user.state,
        "TERMINAL",
        "restored PAPER signal must be reattached to live TP monitoring after restart",
      );
      assert.strictEqual(user.terminalReason, "TP_FILLED");
    },
  );
  ...
  await scenario(
    "RESTART: fresh signal, post-restart tick below strategyInvalidationPrice closes it correctly",
    async () => {
      const rest = mockRestThrowsAlways();
      const { orch, signals, userExecs, globalSignalRepo, strategyOrderRepo } =
        buildStack(runtimes([{ userId: "main", riskUsd: 10, rest }]), false);
      ...
      assert.ok(
        restoredSignal,
        "sanity: signal must still be ACTIVE after restart",
      );
      ...
      await freshOrch.onTick(
        ...
        PCTX,
        historyPost,
        restoredSignal.strategyInvalidationPrice,
        1.0,
        1000,
        7_200_000,
      );
