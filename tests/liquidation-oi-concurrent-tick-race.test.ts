...
function runtimes(specs: Array<{ userId: string; riskUsd: number; rest: BinanceRestLike | null; telegram?: { sendMessage: (t: string) => Promise<void> } }>): () => LiquidationOiUserRuntimeRef[] {
  return () => specs.map((s) => ({ userId: s.userId, riskUsd: s.riskUsd, liquidationOiExecutionEnabled: true, binanceRest: s.rest, telegram: s.telegram ?? { sendMessage: async () => {} } }));
}
function buildStack(getRuntimes: () => LiquidationOiUserRuntimeRef[]) {
  ...
  const orch = new LiquidationOiRuntimeOrchestrator(TEST_STRATEGY_CONFIG, DEFAULT_CAPACITY_MODEL_COEFFICIENTS, globalSignalRepo, strategyOrderRepo, getRuntimes, true, false);
...
async function driveToActive(orch: LiquidationOiRuntimeOrchestrator, symbol: string, now0: number) {
...
async function main(): Promise<void> {
  console.log("Running concurrent-tick race regression tests (production incident)...\n");

  await scenario("REGRESSION: two overlapping onActiveTick calls for the same signal/user must produce exactly ONE close, not zero and not two", async () => {
    const sentPerUser: Record<string, string[]> = { main: [], karo: [] };
    const { orch, activeMain, signals, userExecs } = buildStack(runtimes([
      { userId: "main", riskUsd: 10, rest: mockRestThrowsAlways(), telegram: { sendMessage: async (t) => { sentPerUser.main.push(t); } } },
      { userId: "karo", riskUsd: 1, rest: mockRestThrowsAlways(), telegram: { sendMessage: async (t) => { sentPerUser.karo.push(t); } } },
    ]));
    ...
    await Promise.all([
      activeMain.onActiveTick("BTCUSDT", signal.globalSignalId, "ep-test", "LONG", exitPrice, 4590, 1.0, 1_100_000),
      activeMain.onActiveTick("BTCUSDT", signal.globalSignalId, "ep-test", "LONG", exitPrice, 4590, 1.0, 1_100_001),
    ]);

    for (const userId of ["main", "karo"]) {
      const user = userExecs.docs.find((d: any) => d.userId === userId);
      assert.strictEqual(user.state, "TERMINAL", `${userId} must be terminal`);
      assert.strictEqual(user.terminalReason, "STRATEGY_INVALIDATION");
      const closeMsgs = sentPerUser[userId].filter((t) => t.includes("CLOSED"));
      assert.strictEqual(closeMsgs.length, 1, `${userId} must receive EXACTLY ONE close message, got ${closeMsgs.length}`);
    }
    assert.strictEqual(signals.docs.find((d: any) => d.globalSignalId === signal.globalSignalId).state, "CLOSED");
  });

  await scenario("REGRESSION: two overlapping PAPER TP-hit checks for the same user produce exactly ONE close", async () => {
    const sent: string[] = [];
    const { orch, activeMain, signals, userExecs } = buildStack(runtimes([
      { userId: "main", riskUsd: 10, rest: mockRestThrowsAlways(), telegram: { sendMessage: async (t) => { sent.push(t); } } },
    ]));
    ...
    await Promise.all([
      activeMain.onActiveTick("BTCUSDT", signal.globalSignalId, "ep-test", "LONG", tpPrice, 4590, 1.0, 2_100_000),
      activeMain.onActiveTick("BTCUSDT", signal.globalSignalId, "ep-test", "LONG", tpPrice, 4590, 1.0, 2_100_001),
    ]);
    ...
    assert.strictEqual(closeMsgs.length, 1, `must receive exactly one TP close message, got ${closeMsgs.length}`);
  });
