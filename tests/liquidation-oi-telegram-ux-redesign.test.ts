...
import { formatPrice, formatCompactUsd, formatSignedUsd, formatUtcTime } from "../src/domain/liquidation-oi-strategy/telegram-display-format";
...
const EXCHANGE_INFO = { symbols: [{ symbol: "ADAUSDT", pricePrecision: 5, quantityPrecision: 1, filters: [
  { filterType: "PRICE_FILTER", tickSize: "0.00001" }, { filterType: "LOT_SIZE", stepSize: "0.1", minQty: "0.1" }, { filterType: "MIN_NOTIONAL", notional: "5" },
] }] };
...
function mockRestSuccess(): BinanceRestLike & { calls: string[]; positionAmt: string; tpStatus: string } {
  const calls: string[] = []; let algoId = 1, orderId = 1;
  const state = { positionAmt: "10", tpStatus: "NEW" };
  return {
    calls,
    get positionAmt() { return state.positionAmt; }, set positionAmt(v: string) { state.positionAmt = v; },
    get tpStatus() { return state.tpStatus; }, set tpStatus(v: string) { state.tpStatus = v; },
    ...
    getPositionRisk: async () => [{ symbol: "ADAUSDT", positionAmt: state.positionAmt, entryPrice: "98" }],
  ...
  } as unknown as BinanceRestLike & { calls: string[]; positionAmt: string; tpStatus: string };
}

function runtimes(specs: Array<{ userId: string; riskUsd: number; rest: BinanceRestLike | null; enabled?: boolean; telegram?: { sendMessage: (t: string) => Promise<void> } }>): () => LiquidationOiUserRuntimeRef[] {
  return () => specs.map((s) => ({ userId: s.userId, riskUsd: s.riskUsd, liquidationOiExecutionEnabled: s.enabled ?? true, binanceRest: s.rest, telegram: s.telegram ?? { sendMessage: async () => {} } }));
}
...
async function main(): Promise<void> {
  ...
  await scenario("F.1. formatPrice: ADA-magnitude value never shows raw JS floating point", () => {
    assert.strictEqual(formatPrice(0.19925549538626092), "0.19926");
  });
  await scenario("F.2. formatPrice: BTC-magnitude and ETH-magnitude use sensible precision", () => {
    assert.strictEqual(formatPrice(75164.2), "75,164.2");
    assert.strictEqual(formatPrice(2477.43), "2,477.43");
  });
  await scenario("F.3. formatCompactUsd: K/M compaction, never shows $0.04M for a $40K value", () => {
  ...
  await scenario("F.5. formatUtcTime produces HH:MM:SS UTC", () => {
    assert.match(formatUtcTime(Date.UTC(2026, 8, 17, 8, 40, 12)), /^08:40:12 UTC$/);
  });

  await scenario("B.1. no WATCH Telegram is ever sent -- ENTRY is the first user-facing message", async () => {
    const sent: string[] = [];
    const rest = mockRestThrowsAlways();
    const { orch } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest, telegram: { sendMessage: async (t) => { sent.push(t); } } }]), false);
    await driveToActive(orch, "ADAUSDT", 1_000_000);
    assert.ok(!sent.some((t) => t.includes("WATCH")), "no message may contain the word WATCH");
    assert.ok(sent.some((t) => t.includes("ENTRY")), "ENTRY must still be sent");
  });

  await scenario("C.1. ENTRY message contains the actual persisted globalSignalId", async () => {
    const sent: string[] = [];
    const rest = mockRestThrowsAlways();
    const { orch, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest, telegram: { sendMessage: async (t) => { sent.push(t); } } }]), false);
    await driveToActive(orch, "ADAUSDT", 2_000_000);
    const signal = signals.docs.find((d: any) => d.state === "ACTIVE");
    assert.ok(sent.some((t) => t.includes(signal.globalSignalId)), "ENTRY must contain the persisted globalSignalId");
  });

  await scenario("C.2. CLOSE message contains the same globalSignalId as ENTRY", async () => {
    const sent: string[] = [];
    const rest = mockRestThrowsAlways();
    const { orch, activeMain, signals } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest, telegram: { sendMessage: async (t) => { sent.push(t); } } }]), false);
    ...
    await activeMain.onActiveTick("ADAUSDT", signalId, "ep-test", "SHORT", karoDoc.strategyInvalidationPrice + 1, 4590, 1.0, 3_010_000);
    const closeMsg = sent.find((t) => t.includes("CLOSED"));
    assert.ok(closeMsg, "a CLOSE message must have been sent");
    assert.ok(closeMsg!.includes(signalId), "CLOSE must contain the SAME globalSignalId as ENTRY");
  });
  ...
  await scenario("C.3. TP UPDATE message contains globalSignalId", async () => {
    ...
    const signal = signals.docs.find((d: any) => d.state === "ACTIVE");
    await activeMain.onActiveTick("ADAUSDT", signal.globalSignalId, "ep-test", "SHORT", 102.5, 4590, 1.0, 4_010_000);
    await activeMain.onActiveTick("ADAUSDT", signal.globalSignalId, "ep-test", "SHORT", 101.6, 4590 * 5, 1.0, 4_010_000 + DEFAULT_ACTIVE_LIFECYCLE_CONFIG.oiEfficiencyEvalIntervalMs + 100);
  ...
  await scenario("D.1. ENTRY contains UTC entry time, Risk, Position notional, TP %+$, SL %+$", async () => {
    const sent: string[] = [];
    const rest = mockRestThrowsAlways();
    const { orch } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest, telegram: { sendMessage: async (t) => { sent.push(t); } } }]), false);
    ...
    assert.ok(/TP\s+.*%.*\$/.test(entry), "TP line must contain both % and $");
    assert.ok(/SL\s+.*%.*\$/.test(entry), "SL line must contain both % and $");
  });

  await scenario("A.1. REGRESSION: PAPER never terminates as POSITION_CLOSED_EXTERNALLY via Binance reconciliation, even with a real, flat client", async () => {
    const rest = mockRestSuccess();
    rest.positionAmt = "0";
    const { orch, positionLifecycle, userExecs } = buildStack(runtimes([{ userId: "brother", riskUsd: 1, rest, enabled: false }]), true);
  ...
  await scenario("A.2. restart preserves an ACTIVE PAPER position and resumes virtual monitoring", async () => {
    const rest = mockRestSuccess();
    rest.positionAmt = "0";
    const { orch, globalSignalRepo, strategyOrderRepo, positionLifecycle, userExecs } = buildStack(runtimes([{ userId: "friend", riskUsd: 1, rest, enabled: false }]), true);
    await driveToActive(orch, "ADAUSDT", 7_000_000);
    await recoverLoxOnRestart(globalSignalRepo, strategyOrderRepo, positionLifecycle, orch.getWatchManager(), runtimes([{ userId: "friend", riskUsd: 1, rest, enabled: false }]), () => {}, 7_015_000);
    const after = userExecs.docs.find((d: any) => d.userId === "friend");
    assert.strictEqual(after.mode, "PAPER");
    assert.strictEqual(after.state, "ACTIVE", "restart must NEVER treat a PAPER user's flat real account as a close");
  });

  await scenario("A.3. REAL external-close reconciliation still works unchanged", async () => {
    const rest = mockRestSuccess();
    const { orch, positionLifecycle, userExecs } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest, enabled: true }]), true);
    ...
    assert.strictEqual(before.state, "ACTIVE", "sanity: entry must have succeeded before we simulate a later external close");
    rest.positionAmt = "0"; rest.tpStatus = "NEW"; // simulate the position closing on Binance AFTER entry, with no provable cause
  ...
  await scenario("A.4. mixed PAPER/REAL cannot cross-close", async () => {
    const karoRest = mockRestSuccess(); karoRest.positionAmt = "10";
    const artakRest = mockRestSuccess(); artakRest.positionAmt = "0";
    const { orch, positionLifecycle, userExecs } = buildStack(runtimes([{ userId: "karo", riskUsd: 1, rest: karoRest, enabled: true }, { userId: "artak", riskUsd: 5, rest: artakRest, enabled: false }]), true);
    ...
    assert.strictEqual(karo.state, "ACTIVE", "Karo (REAL, still open) must remain ACTIVE");
    assert.strictEqual(artak.state, "ACTIVE", "Artak (PAPER) must remain ACTIVE regardless of his real account being flat");
  });

  await scenario("N.1. each user's Telegram receives ONLY that user's own ENTRY -- no cross-leak", async () => {
    const karoSent: string[] = []; const artakSent: string[] = [];
    const karoRest = mockRestThrowsAlways(); const artakRest = mockRestThrowsAlways();
    const { orch } = buildStack(runtimes([
      { userId: "karo", riskUsd: 1, rest: karoRest, telegram: { sendMessage: async (t) => { karoSent.push(t); } } },
      { userId: "artak", riskUsd: 5, rest: artakRest, telegram: { sendMessage: async (t) => { artakSent.push(t); } } },
    ]), false);
    await driveToActive(orch, "ADAUSDT", 10_000_000);
    assert.ok(karoSent.some((t) => t.includes("Karo")), "Karo's own message must show Karo's own display name");
    assert.ok(!karoSent.some((t) => t.includes("Artak")), "Karo must never receive a message mentioning Artak");
